...
- [ ] Complete subclass and armor-ability effects. The authored `armorAbilities` table carries all 18
      ...
      **Progress 2026-09-19 (21st matrix, `MONSTER_ANALYSIS_CHALLENGE.md`):** `Challenge` is
      ...
      crown's old description line rather than an empty choice): the remaining three abilities
      (Mage's two, Duelist's `ElementalStrike`) each need a system this port does not have, and
