...
- [ ] Check Rollup code-splitting (`build.rollupOptions.output.manualChunks`, https://rollupjs.org/configuration-options/#output-manualchunks). Every build currently warns that chunks exceed 500 kB after minification. Before adopting, verify the split is compatible with this port's `file://` constraint: `vite.config.ts` bundles to a single IIFE `game.js` classic script with all PNG/audio inlined as `data:` URIs (`assetsInlineLimit`), precisely so nothing is ever fetched - a multi-chunk output needs one `<script>` tag per chunk (via `tools/emit.mjs`) and reintroduces the load-order/fetch surface the single script exists to avoid. If the inlined assets dominate the bundle, the warning may be accepted rather than fixed; either way record the decision in `vite.config.ts`'s own comment. **Complexity: S.**
- [ ] Check MWL "native" compiler usage. `tools/compile-mwl.mjs` already drives the framework's own `mwg/mwl` library (`compileSources`, `validateCatalog`, `contentCatalog` - see the file's header and section 11A's P11 resolution), with game-owned cross-table validators and custom emit on top. Audit what the driver still hand-rolls against what the installed `mwg` now ships (`mwg/tools/mwl.mjs` CLI, the `classic-html` post-build used by `tools/emit.mjs`): adopt wherever the framework covers the port's needs, and record each remaining hand-rolled piece with its reason (as P11's CLI-vs-library split already is) rather than leaving it implicit. **Complexity: S.**
