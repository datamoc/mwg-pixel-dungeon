...
type Command =
	| { kind: 'attack'; attacker: Combatant; defender: Combatant; randomId: number; magic: boolean; surprise: boolean }
...
const rule: SimulationRuntimeRule<State, Command, Event, SpdActor> = (_state, command) => {
	switch (command.kind) {
		case 'attack': {
			...
			const resolution = resolveAttack(command.attacker, command.defender, random, command.magic, command.surprise);
...
export function runAttackResolution(attacker: Combatant, defender: Combatant, random: SimulationRandom, magic = false, surprise = false): AttackResolution {
	...
	try {
		return (dispatch({ kind: 'attack', attacker, defender, randomId, magic, surprise }) as { type: 'attack-resolution'; resolution: AttackResolution }).resolution;
	} finally {
