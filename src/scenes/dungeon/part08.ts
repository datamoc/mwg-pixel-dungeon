...
import { BUFF_DURATION, INFINITE_EVASION, NEGATIVE_BUFFS, absorbShield, addBuff, reigniteBuff, rollDamage, rollHit, setBleeding, stoneGlyphReduction, type Creature } from '../../combat';
import { liveStats } from '../../monsters';

/** DungeonScene methods, moved verbatim from `dungeonScene.ts` (group `part08`). Each takes the scene as `this`;
 * `dungeonScene.ts` merges them back onto the class prototype. */
export const part08Methods = {
	/**
	...
	absorbHeroDamage(this: DungeonScene, amount: number, magical = false): number {
