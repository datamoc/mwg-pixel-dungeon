...
import { armorReductionRange, isClassArmorId, weaponDamageRange } from '../../items/catalog';
...
import { DOOR, GAME_KIND_CODES, HIGH_GRASS, SOLID, TERRAIN_KINDS, TILE, WALL, WATER } from '../../dungeonConstants';
import { REGION_GRASS, REGION_WATER, generateSpdDungeon, regionForDepth, type Region } from '../../genericDungeon';
import { INFINITE_EVASION, addBuff, baseCreature, rollHit, type BuffId, type Creature, type Step } from '../../combat';
...
/** DungeonScene methods, moved verbatim from `dungeonScene.ts` (group `part01`). Each takes the scene as `this`;
 * `dungeonScene.ts` merges them back onto the class prototype. */
export const part01Methods = {
	...
	spawnMonster(this: DungeonScene, kind: AnyMonsterId, at: Step, restoring = false, mimicLoot?: string, isAlly = false, allyKind?: 'mirror' | 'sheep' | 'ward' | 'earthGuardian' | 'lotus' | 'ghost' | 'ninjaLog' | 'spiritHawk' | 'afterImage' | 'shadowClone' | 'prismatic', championEligible = false): Creature {
