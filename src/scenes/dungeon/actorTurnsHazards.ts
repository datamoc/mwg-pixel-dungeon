...
import { chasmJumpLatched, chasmStepNeedsConfirm, setChasmJumpLatched } from '../../simulation/chasmJump';
...
export const actorTurnsHazardsMethods = {
	/**
	...
	takeHeroTurn(this: DungeonScene, move: Step): void {
		...
		//The reach is the weapon class's own `RCH` (spear/glaive 2, whip 3, `weaponCombatRules`) plus Projecting's
		//`round(multiplier)`; before, only Projecting lengthened it and a spear reached one cell like a dagger.
		const weaponReach = weaponCombat(this.weaponMeleeKey(), this.weaponTier, 0).reach;
		const reach = weaponReach + (this.weaponAffix === 'projecting' ? Math.round(this.genericProcMultiplier()) : 0);
		if (reach > 1) {
		...
		//transition half.
			...
			//lets the same dispatch through the second time.
			if (chasmStepNeedsConfirm(this.isChasmCell(target.x, target.y), !!this.hero.buffs['levitation'], chasmJumpLatched(this))) {
				this.confirmChasmJump(move);
				return;
			}
			const from = { x: this.hero.x, y: this.hero.y };
 			this.moveTo(this.hero, target);
 			this.leaveDoor(from.x, from.y, this.hero);
			...
			if (!this.justDescended) this.searchForSecrets(false);
	 ...
	 */
	/** `Plant.activate(null)` (tag `v3.3.8`): empty-cell roots still fire the plant's
	 * cell effect. Java's Firebloom/Rotberry seed their Fire/ToxicGas blobs and
	 * Dewcatcher/Seedpod scatter loot without needing a `Char`; the old port only
	 * dispatched when an occupant existed, so those four roots silently did nothing. */
	triggerEmptyPlantAt(this: DungeonScene, x: number, y: number): void {
		const cell = this.level.index(x, y);
		const kind = this.plantKindAt(cell);
		if (!kind) return;
		if (this.portedPaint) {
			const index = this.portedPaint.plants.findIndex((plant) => plant.pos === cell && !plant.kind.startsWith('wellWater:'));
		...
		switch (kind) {
			...
			case 'rotberry': this.plantGas.seed(x, y, 100); break;
			case 'dewcatcher': this.dropPlantNeighbourLoot(x, y, 3, 6, 'dew'); break;
			case 'seedpod': this.dropPlantNeighbourLoot(x, y, 2, 4, 'seed'); break;
		}
		this.featuresMap?.setLayerData('features', this.featureFrames());
	},
	...
	triggerPortedPlantAt(this: DungeonScene, x: number, y: number): void {
		...
		//`Plant.trigger()` (tag `v3.3.8`) calls `Hero.interrupt()` before every plant's
		//`wither()`/`activate()` pair, not only for Fadeleaf. The port's queued click-travel
		//is its equivalent of Java's current action, so clear it before any plant effect.
	...
	heroPlantContext(this: DungeonScene): HeroPlantContext {
		return {
			...
			returnToPreviousFloor: () => {
				...
				//current floor's cell. enterLevel resolves this sentinel after generation.
				this.beaconArrival = { x: -1, y: -1 };
				this.enterLevel();
				return true;
			},
	 ...
	 */
	/** `Chasm.heroJump()` (`Chasm.java` 60, tag `v3.3.8`): the yes/no window before a
	 * voluntary chasm step, worded with Java's own `levels.features.chasm.*` strings (already
	 * in the generated catalog, so no `port.*` strings needed). "Yes" latches the confirmation
	 * and re-enters the same turn (`missileThrowNeedsConfirm`'s re-entry twin); "no" just
	 * closes, and the queued path is cancelled either way (Java's `interrupt()`). The 200ms
	 * answer guard is Java's `elapsed > 0.2f` anti-misclick. Levitation skips the warning like
	 * Java's `flying` bypass - except Java still warns when the levitation is about to detach
	 * mid-step, which this port cannot time and so treats as flying (documented below).
	 */
	confirmChasmJump(this: DungeonScene, move: Step): void {
		const fromX = this.hero.x;
		const fromY = this.hero.y;
		showConfirmWindow(
			this.gameWindows,
			t('levels.features.chasm.chasm'),
			t('levels.features.chasm.jump'),
			t('levels.features.chasm.yes'),
			t('levels.features.chasm.no'),
			() => {
				//Java's `onSelect`: the hero must still be where the window opened.
				if (this.hero.x !== fromX || this.hero.y !== fromY) return;
				setChasmJumpLatched(this, true);
	...
	fallThroughChasm(this: DungeonScene, x: number, y: number): boolean {
		...
		//`Chasm.heroFall()` resets `jumpConfirmed` first, before the feather check.
		setChasmJumpLatched(this, false);
		//`Chasm.heroFall()` plays `FALLING` as the hero drops; the bundled asset was
		//already shipped but nothing cued it.
		runState.audio.cue('falling');
	 ...
	 * itself. The fall plays Java's `FALLING` cue (`Chasm.heroFall()`, wired in
	 * `fallThroughChasm`) and the landing shakes (`PixelScene.shake(4, 1f)`, `Chasm.java`
	 * 143, before the Cripple it also applies); `ElixirOfFeatherFall.FeatherBuff` cancels
	 * the whole thing outright, the marker consumed by `consumeFeatherFall()`. Without it,
	 * the two mechanical consequences are a `Cripple` application and upfront
	...
	const visibleAllyTarget = findEnemyAllyFlow(monster, this.creatures,
		...
		if (monster.kind === 'sentry') {
			...
			takeSentryTurnFlow(monster, this.hero, {
				...
				warmup: monster.sentryWarmup ?? monster.sentryInitialWarmup,
	...
	takeEarthGuardianTurn(this: DungeonScene, guardian: Creature): void {
		...
		if (next) {
			...
			this.moveTo(guardian, next);
			this.leaveDoor(from.x, from.y, guardian);
		}
	},
	...
	takeWardTurn(this: DungeonScene, ward: Creature): void {
		...
		if (target) {
			...
			//`simulation/deathBursts.ts`) plus the attacker flash - the same
