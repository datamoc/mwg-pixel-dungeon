...
import { LETHAL_HASTE_COOLDOWN, lethalHasteDuration, soulSiphonCharge } from '../../talentEffects';
...
import { addBuff, reigniteBuff, rollHit, type BuffId, type Creature, type GroundItem } from '../../combat';
...
/** DungeonScene methods, moved verbatim from `dungeonScene.ts` (group `part09`). Each takes the scene as `this`;
 * `dungeonScene.ts` merges them back onto the class prototype. */
export const part09Methods = {
	...
	kill(this: DungeonScene, creature: Creature, cause: 'foe' | 'trap' | 'fire' | 'poison' | 'hunger' = 'foe'): void {
	...
	if (creature.isHero && this.openResurrectWindow()) return;
		...
		if (boss && boss.kind === creature.kind) {
			...
			//call site here, which passes a `t('port.â€¦')` key). It now authors a message key
