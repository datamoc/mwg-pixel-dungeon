 ...
 *
 * Ported so far: the Warrior's three. Still to port, each needing its own systems: the Mage's
 * (`ElementalBlast` and `WildMagic` need per-wand blast factors and a wand-randomization pass;
 * `WarpBeacon` needs a beacon actor and window), the Rogue's (`SmokeBomb`/`DeathMark` need the real
 * `SmokeScreen`/`DeathMark` buff pair; `ShadowClone` needs an ally actor), the Huntress's
 * (`SpectralBlades` needs the spirit-blade projectile; `NaturesPower` a growing-power buff;
 * `SpiritHawk` an ally actor) and the Duelist's (`Challenge` needs a duel tracker; `ElementalStrike`
 * the four blade imbuements; `Feint` a feint buff). See `PORT_COVERAGE.md`'s armor-ability rows.
 */
const PORTED_ARMOR_ABILITIES: ReadonlySet<string> = new Set(['heroicleap', 'shockwave', 'endure']);
...
/**
 * `ArmorAbility.chargeUse(hero)` and `HeroicLeap.chargeUse(hero)` together: the base cost scaled by
 * `HEROIC_ENERGY`, with Heroic Leap's own extra `0.84^points` in `DOUBLE_JUMP` discount while its
 * `DoubleJumpTracker` is up (i.e. while the hero's *next* leap is the discounted one). The
 * discount is keyed off the ability id here rather than left to the caller, because it is an
 * override on `HeroicLeap` alone - every other ability inherits the base implementation.
 */
export function armorChargeUse(
	def: ArmorAbilityDef,
	options: { heroicEnergyRank: number; doubleJumpArmed?: boolean; doubleJumpRank?: number },
): number {
	...
	if (def.id === 'heroicleap' && options.doubleJumpArmed) {
		chargeUse *= Math.pow(0.84, Math.min(4, Math.max(0, options.doubleJumpRank ?? 0)));
	}
	return chargeUse;
}
