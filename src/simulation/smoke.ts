 ...
 * distance-2 flood *including* the center; every flood key costs 40 from a
 * 1000-volume budget, and the unplaced share piles onto the center on top of its
 * own 40 (so the center totals 40 plus the remainder). `SmokeScreen`
...
export function smokeBombSeedPlan(
	...
	//Java's loop seeds 40 on every reached cell including the center, then piles the
	//remainder on top (`Blob.seed(i, 40)` per cell, `centerVolume -= 40` each, plus a
	//second `Blob.seed(cell, centerVolume)` at the center). The plan keeps the center
	//out of `seeds` so the scene seeds it once, carrying its own 40 in `centerVolume`.
	const seeds = flood
		.filter((cell) => cell.x !== cx || cell.y !== cy)
		.map((cell) => ({ ...cell, volume: SMOKEBOMB_VOLUME }));
	const centerVolume = SMOKEBOMB_VOLUME
		+ Math.max(0, SMOKEBOMB_CENTER_BUDGET - SMOKEBOMB_VOLUME * flood.length);
