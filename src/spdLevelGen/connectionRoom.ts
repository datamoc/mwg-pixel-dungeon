...
const CHANCES: Record<number, number[]> = Object.fromEntries(
	MWL_TABLE_ROWS('connectionRoomChanceRows', 'depth').map((row) => [
		Number(row.depth),
		(Array.isArray(row.chances) ? row.chances.map(Number) : []),
	]),
);
