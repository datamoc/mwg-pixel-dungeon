...
2. If `Codex-in-chrome` isn't connected (extension not paired to this session), the
...
4. The title screen and class-select buttons need a real pointer event to advance past.
   With `Codex-in-chrome`, dispatch a `PointerEvent` via `javascript_tool` rather than
...
specifier in `package.json` (`"mwg": "npm:@datamoc/mw_games@^0.7.7"`) since every source
...
- **`Codex-in-chrome`** for the browser verification below. When it is unavailable, say so
