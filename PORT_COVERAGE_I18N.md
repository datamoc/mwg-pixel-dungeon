...
| `assets/messages/**/*.properties`: 9 domains x English + 22 locales, 207 files at tag `v3.3.8` | `src/generated/spdMessages.ts`, built by `tools/i18n-extract.mjs` (`npm run i18n`) | **Ported: 4,834 SPD message keys x 23 language catalogs** (2026-09-25). The exact `v3.3.8` tag contains 4,830 distinct English base keys; four older keys still referenced by the port are included from the optional legacy source fallback. The pinned tag is the primary source for every key and translation, and the older source is consulted only when a referenced key is missing from the tag. All 22 target locales are read from the tag. The complete catalog is compiled into the bundle because the built page runs from `file://` and cannot fetch locale files on demand.
...
| The complete key set to ship: every target-tag base key plus every key referenced by the port | `tools/i18n-extract.mjs`'s `referencedKeys()`, audited before any write | **Ported (2026-09-25):** extraction unions the exact v3.3.8 base corpus with referenced keys, then validates each extra against the optional legacy source. Unresolved keys fail before writing. All 4,830 target base keys are included; the generated catalog has 4,834 keys including four legacy references. The committed output is regenerated from the tag, not a moving checkout.
...
| Referenced Java keys missing from the older live checkout but present at pinned tag `v3.3.8` (`MagesStaff.imbue_cursed`; `MagesStaff.java`'s warning shown when imbuing an unidentified spare wand) | `tools/i18n-extract.mjs` and `src/generated/spdMessages.ts` | **Ported (2026-09-24):** the extractor now uses `--legacy-spd-root` as a fallback for referenced base keys and their locale translations, while retaining the live checkout's values wherever it has them. This restores SPD's real English warning and translations in all 22 shipped locales without renaming the Java key or hand-maintaining text. Keys absent from both sources still fail the transactional audit. Verified against `v3.3.8` and `4.0.0-beta`; the prior extraction failed on this valid key because its primary checkout predates it. |
