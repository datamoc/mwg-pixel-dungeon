...
// Goo's enrage and Brute's rage (`simulation/combat.ts`'s `liveStats`): state-dependent combat
// numbers no static monster row can hold. Emitted as a generated module like the buff/immunity
// data above, so the framework-free simulation stays MWL-import-free.
const gooStateStats = {};
for (const row of tableRows('gooStateStats')) {
  const monster = String(row.monster);
  const state = String(row.state);
  if (!monsterIds.has(monster)) throw new Error(`MWL gooStateStats references unknown monster: ${monster}`);
  if (!['healthy', 'enraged'].includes(state)) throw new Error(`MWL gooStateStats has unknown state: ${state}`);
  if (gooStateStats[state] !== undefined) throw new Error(`MWL gooStateStats has a duplicate row: ${state}`);
  gooStateStats[state] = {
    accuracy: Number(row.accuracy),
    evasion: Number(row.evasion),
    damageMin: Number(row.damage_min),
    damageMax: Number(row.damage_max),
  };
}
for (const state of ['healthy', 'enraged']) {
  if (!gooStateStats[state]) throw new Error(`MWL gooStateStats is missing state: ${state}`);
}
// The healthy state duplicates `monsters.mwl`'s goo row (spawn uses the row, combat uses this
// table) - pin them together so a future edit to one cannot silently diverge from the other.
const gooBase = nodes.find((node) => node.tag === 'monster' && node.attributes?.id === 'goo')?.attributes;
for (const [key, attribute] of [['accuracy', 'accuracy'], ['evasion', 'evasion'], ['damageMin', 'damage_min'], ['damageMax', 'damage_max']]) {
  if (Number(gooBase?.[attribute]) !== gooStateStats.healthy[key]) {
    throw new Error(`MWL gooStateStats healthy ${key} diverges from the monsters.mwl goo row`);
  }
}
const bruteRageDamage = {};
for (const row of tableRows('bruteRageDamage')) {
  const monster = String(row.monster);
  if (!monsterIds.has(monster)) throw new Error(`MWL bruteRageDamage references unknown monster: ${monster}`);
  if (bruteRageDamage[monster] !== undefined) throw new Error(`MWL bruteRageDamage has a duplicate row: ${monster}`);
  bruteRageDamage[monster] = [Number(row.damage_min), Number(row.damage_max)];
}
for (const value of [...Object.values(gooStateStats).flatMap((state) => Object.values(state)), ...Object.values(bruteRageDamage).flat()]) {
  if (!Number.isFinite(value)) throw new Error('MWL monster state stats must be finite numbers');
}

// The summon assigns stats by fist type, so both types Java can produce must be authored.
const yogFistSummonTypes = new Set(tableRows('yogFistSummonStats').map((row) => String(row.type)));
for (const type of ['default', 'rusted']) {
  if (!yogFistSummonTypes.has(type)) throw new Error(`MWL yogFistSummonStats is missing type: ${type}`);
}
...
const artifacts = compileAndEmitSources(sources, {}, {
  ...
  artifacts: {
    ...
    'mwlMonsterStateStats.ts': `// Generated by tools/compile-mwl.mjs from src/content/actor-rules.mwl.\nexport const GOO_STATE_STATS = ${JSON.stringify(gooStateStats, null, 2)} as const;\nexport const BRUTE_RAGE_DAMAGE = ${JSON.stringify(bruteRageDamage, null, 2)} as const;\n`,
  },
});
const ARTIFACT_PATHS = {
  ...
  'mwlMonsterStateStats.ts': ['src', 'simulation', 'mwlMonsterStateStats.ts'],
};
