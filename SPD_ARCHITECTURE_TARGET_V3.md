...
| SPD-ADR-010 | Business families are refactored only after a data/function/method analysis; inheritance is never the default | Adopted as a going-forward rule for this plan. **Updated 2026-09-25:** the status here said "no family has had the full matrix produced yet", which stopped being true in 2026-09 - `garbage/MONSTER_ANALYSIS_*.md` now holds **42** matrices (BACKLOG B8, through the forty-second on 2026-09-20), so the rule is in force rather than aspirational. The families that still owe theirs are what coord task T62 tracks. |
