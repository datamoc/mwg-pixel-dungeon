...
| `SummonElemental.onCast()` / `InvisAlly` | `useSummonElemental`, `spawnMonster` | Simplified (2026-09-14): the existing alchemy result now summons an allied newborn elemental into an adjacent free cell and spends a turn only when space exists. Java's imbue picker (Liquid Flame/Frost/Recharging/Transmutation), mature elemental subtypes, temporary ally buff, and summon/teleport presentation remain unmodeled; the port uses its existing newborn fire-elemental actor as the playable default. |
...
| `CurseInfusion.onItemSelected()` / `InventorySpell.usableOnItem()` | `useCurseInfusion`, generic item picker, `getWeaponCurses`/`getArmorCurses` | Simplified (2026-09-14): the existing alchemy result now curses a carried weapon, armor, or wand, replaces weapon/armor affixes with a real negative curse, prevents a second infusion bonus, and grants one persistent level. Java also targets equipped gear and MagesStaff/SpiritBow, models the bonus separately so cleansing removes it, and supplies curse/infusion presentation; this port's picker and item model do not yet expose those seams. |
| `MagicalInfusion.onItemSelected()` / `upgradeItem()` | `useMagicalInfusion`, `upgradeItem`, generic item picker | Simplified (2026-09-14): the existing alchemy result now upgrades a selected carried weapon, armor, wand, or ring by one level while preserving its affix through the shared MWG upgrade operation. Java also exposes equipped gear and other upgradable item classes, and separately preserves/removes a wand's Curse Infusion bonus; those distinctions are not represented by this port's current picker/item model. |
...
id has no catalogue entry to validate against) and an item-suite pin. Java stocks a wider
generated assortment - that is section 1's shop-pricing remainder, not this table.
...
### mwg usage audit (2026-09-12 baseline, refreshed against installed 0.9.0 on 2026-09-13)
...
| `HighGrass.trample()`: Huntress furrows, repeat-trample handling, and real loot odds (1/25 seed, 1/6 dew, "naturalism level 0") | `trampleHighGrass`, `furrowedGrass` | Ported for the Huntress-only `FURROWED_GRASS` state: Huntress steps create and preserve the furrow; another hero clears it without drops. The state persists per floor. Ordinary seed/dew/berry rolls are ported; the Java `freezeTrample` re-entry guard is unnecessary because this port's item spawning cannot synchronously retrigger movement. The compact live terrain keeps the high-grass visual while the saved state carries the raw distinction, so the dedicated furrowed visual variant is not ported. |
