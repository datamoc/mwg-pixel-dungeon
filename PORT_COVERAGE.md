...
| The remaining two abilities: Mage `ElementalBlast`/`WildMagic` | - | **Not ported**, deliberately, and not offered: each needs systems this port does not have yet (per-wand blast factors and a wand-randomization pass). Their data rows exist - names, charge costs, targeting modes and T4 talent lists - so the ported/non-ported split is one set in `armorAbilities.ts`, and the choice panel offers only what can actually run. **This row previously also listed Huntress `SpiritHawk`, Duelist `Feint`, `Challenge` and `ElementalStrike`, and Rogue `ShadowClone` as not ported - all five are ported now (see their own rows); the `SpiritHawk` half of that claim was already stale before its pass, not just made stale by it.** **2026-09-19:** `ElementalBlast`'s pure arithmetic is ported and pinned (`simulation/mageAbilities.ts`, `tools/verifyArmorAbilities.mjs` inside `test:simulation`) - the per-wand factors, `ELEMENTAL_POWER`/`BLAST_RADIUS`, the roomiest-cardinal aim, damage, the Transfusion undead/ally splits, Corrosion, nine buff durations, the Regrowth grass chance, knockback and `REACTIVE_BARRIER` - but the ability stays unoffered: firing reads the blast's wand off an imbued `MagesStaff`, and the imbue system (`AC_IMBUE`) does not exist here. `WildMagic` is untouched. |
