...
| `Spinner` poison resistance (half duration) and `Web` immunity | - | Not ported: the buff pipeline models immunities only, and no resistance-halving mechanism exists - full-duration poison sticks to a spinner. Web immunity is moot in practice (nothing reads web volume except the root-each-turn loop, and webs land on the hero at range) - recorded by `MONSTER_ANALYSIS_BAT_ALBINO_SWARM_SPINNER.md`. |
