...
| `PotionOfFrost.shatter()` (`Freezing` blob: extinguish fire, `Chill`, eventual `Frost` immobilize) | `quaffPotion`'s `potionFrost` branch | Simplified - this was the last generated potion id with no branch of its own (it fell through to Purity's effect). No full Freezing blob or distinct Frost immobilize exists here, so the cast is target-centred (no thrown-cell picker), applies the real 10-turn `Chill` slowdown, extinguishes Burning, and applies Java's Elemental harmful-buff damage when appropriate. Type-check/build only; browser verification owed per ROADMAP.md section 10. |
