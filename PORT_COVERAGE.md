...
| `Buff.durations` (`src/content/buff-rules.mwl`, authored data): `Burning` 8, `Cripple` 10, `Paralysis` 10, `Roots` 5, `Daze` 5, `Ooze` 20, `Levitation` 20, `Invisibility` 20, `Chill` 10 at tag `v3.3.8` | `BUFF_DURATION` / `mwlBuffDurations.ts` | **Four values are shorter than Java's and nothing recorded why - flagged 2026-09-12, not changed.** The port's table matches Java exactly for `daze` (5), `ooze` (20), `levitation` (20), `invisibility` (20) and `chill` (10), but `burning` is 3 against Java's `Burning.DURATION = 8`, `cripple` 4 against 10, `paralysis` 3 against 10 and `roots` 3 against 5 - all four shortened by roughly half to two thirds, all four among the debuffs that actually hurt, and none of them derivable from any Java source this port has read. `poison` (6) and `bleeding` (0) have no Java `DURATION` constant at all (their sources call `set(duration, damage)`), so those two are the port's own convention rather than a mismatch. Under this project's own rule that values come from the real Java source, this is either deliberate port tuning that needs saying so or an error that needs correcting - recorded here with the exact numbers rather than silently re-balanced, because changing four core debuff durations is a balance-wide change that belongs in its own deliberate pass (and would want the user's call, not an agent's). |
