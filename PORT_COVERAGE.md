...
| `Ratmogrify` (`baseChargeUse = 50`, `talents()` = `RATSISTANCE`/`RATLOMACY`/`RATFORCEMENTS` + `HEROIC_ENERGY`) | `grantRatmogrify`, `ratmogrifyChargeUse`, `useRatmogrify` | Partially ported. The charge cost is now real (50, scaled by `HEROIC_ENERGY`) and **a real turn-cost bug was fixed in the same pass**: `useRatmogrify` both spent its own turn and returned `true` to the action adapter that spends one, so every Ratmogrify cost two turns. The six-turn transformation itself keeps its documented simplification (no `TransmogRat` actor, so the sprite and kind stay), which is exactly why the three rat talents used to grant no tier-4 pool here. **2026-09-19:** Ratmogrify now has its own `class: "any"` row in `talent-rules.mwl` (granted by the Rat King to any class, never offered), so the tier-4 tab opens with all three rat talents plus `HEROIC_ENERGY`, and `RATLOMACY` (permanent ally on transformed rats) and `RATFORCEMENTS` (ally-rat spawn on empty self-cast) run off their ranks. Still open: `RATSISTANCE`'s damage factor (needs the missing `TransmogRat` actor's damage path), and the same pass fixed the message key - Java derives it from the ability class's package, so unlike the eighteen class abilities Ratmogrify's key has no class segment (`actors.hero.abilities.ratmogrify`, `actors.properties:609`). |
