...
| The other nine abilities: Mage `ElementalBlast`/`WildMagic`, Rogue `SmokeBomb`/`ShadowClone`, Huntress `NaturesPower`/`SpiritHawk`, Duelist `Challenge`/`ElementalStrike`/`Feint` | - | **Not ported**, deliberately, and not offered: each needs systems this port does not have yet (per-wand blast factors and a wand-randomization pass; the real `Blindness` buff and the `NinjaLog` ally; ally actors for the clone and the hawk; a growing-power tracker wired into the SpiritBow and the hero's speed; a duel tracker; the four blade imbuements; a feint buff and its after-image). Their data rows exist - names, charge costs, targeting modes and T4 talent lists - so the ported/non-ported split is one set in `armorAbilities.ts`, and the choice panel offers only what can actually run. |
