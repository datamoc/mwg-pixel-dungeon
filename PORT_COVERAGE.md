...
| `AntiMagic.drRoll()`/`Char.damage()` (armor glyph: reduce listed magical damage) | `absorbHeroDamage(amount, magical)`, `zapHero`, `eyeBeamTurn` | Ported for the represented magical ranged sources: the Java level/Arcana-scaled `NormalIntRange(level, 3+1.5*level)` reduction is applied before shields. The port marks its explicit magic-bolt/death-gaze callers; unrepresented Java spell/bomb/trap source classes remain outside this seam. Type-check/build/tests pass; browser verification owed per ROADMAP.md section 10. |
