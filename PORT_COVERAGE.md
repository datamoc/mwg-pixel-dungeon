...
| `WandOfRegrowth.onZap()` / `chargesPerCast()` / degradation counters | `useRegrowthWand`, `regrowthChargeLimit`, `useSpecial` wand branch, run save/load | **The bolt path and the Lotus placement are now exact (2026-09-12); only the affected *shape* is still an approximation.** Exact: the centre line is Java's `bolt.path` - `Roguelike.traceLine` from the hero to the aimed cell, which is the wall-ignoring straight line because an uncursed wand's own `collisionProperties` is `WONT_STOP` (`WandOfRegrowth.java` 66-67: "only used for targeting, actual projectile logic is Ballistica.STOP_SOLID"), replacing a hand-written cross-product band; the surviving cells are shuffled at Java's own point in the sequence (`WandOfRegrowth.onZap()` shuffles before the Lotus and the grass line - Java's own order comes from a `HashSet`, which no port reproduces, and a shuffle of any fixed order is the same uniform permutation); and the 3-charge Lotus takes the *aimed* cell when it is free, otherwise the first free cell walking that path backwards from its end, replacing a "nearest eligible" search. Browser-verified live (`tools/scratch/regrowth-path-livecheck.mjs`, 7 assertions): with an open three-cell run and a monster at its far end, the Lotus lands on the cell immediately back from the monster, collinear with hero and monster, on the cell Java's backwards walk picks, and the same path places grass along all three cells. Still approximate: the cell *set* is a Chebyshev circle of radius `2 + 2*charges` centred on the aimed cell, where Java builds `ConeAOE` over that ballistica - a circular sector of `2 + 2*charges` range and `20 + 10*charges` degrees, built by casting a ray every 0.5 degrees across the arc, also casting at `radius - 1` when the radius is at least 4, then unioning `Ballistica.subPath(1, dist)` for each struck cell with `STOP_SOLID | STOP_TARGET` (`WandOfRegrowth.java` 246-252, `mechanics/ConeAOE.java`). Porting that is a shared task: `ConeAOE` also backs `WandOfFireblast` (`3 + 2*charges` range, `30 + 20*charges` degrees, +`IGNORE_SOFT_SOLID`), `DM300`'s 30-degree gas check, and seven features this port has not reached (ElementalStrike, SpectralBlades, ElementalBlast, Shockwave, VaultSentry, TalismanOfForesight, PotionOfDragonsBreath, CursedWand) - so it wants its own pass, and it is the reason the roadmap still lists "exact cone targeting". Note also that `mwg/roguelike`'s `coneCells` is *not* a substitute: it is a snapped-eight-direction spray whose width grows linearly, not an angular sector (recorded as proposal P13 in `ROADMAP.md`). The rest of the row stands: the real 1-3 charge cost (`ceil(30% of current charges)`), level-scaled degradation limit, `round((3.67 + level/3)*charges)` grass budget, `4*charges` Roots duration, furrow chance after the limit, Seedpod/Dewcatcher chances, and generated seed activation are reproduced. Still missing besides the cone itself: Java filters out cells holding an `IMMOVABLE` character before the roots pass (this port roots whatever is there), and the Dwarf King's boss-challenge-badge flag has no equivalent here. |
