...
- **MysteryMeat / ChargrilledMeat energy and effect** — `consumableStats` rows plus
  `applyMysteryMeatEffect` in `src/items/consumables.ts`. **Ported 2026-09-19 (19th matrix,
  ...
  **Simplified:** the Slow case is unmodeled - no speed-factor buff exists in this port - so
  2 of 5 rolls are silent instead of Java's 1 of 5. Exotic foods (`Berry`, `Blandfruit`,
  `FrozenCarpaccio`, `PhantomMeat`, `SupplyRation`, `SmallRation`) remain **Not ported** - none
  is generated or dropped anywhere in this port.
...
| `Chasm.java`'s `heroLand()` (fall-damage consequences, once a floor/chasm cell exists to fall through) | `fallThroughChasm`/`landFromChasm` | Ported: `Cripple`, the real `Bleeding` intensity formula, and the real `max(HP/2, NormalIntRange(HP/2, HT/4))` upfront damage, run through the same hero-damage absorption pipeline (Tenacity/Barrier/Iron Will/Deathless Fury) every other source uses; a chasm death correctly triggers `kill()`. Not ported: Bleeding's source-class death badge distinction and blood-splash presentation, the landing sound, and camera shake (no camera-shake system exists). **Correction, 2026-09-19: `FeatherFall`'s fall-negation (`consumeFeatherFall`) is not missing** - it was already ported and is called from this same `landFromChasm` site (see that class's own row); this row's older claim predates that and was never updated. `Chasm.heroJump()`'s confirmation dialog is UI-only and isn't needed given this port has no equivalent modal-confirmation system for movement. |
...
| The Cleric spell system entire (30-spell `ClericSpell` roster, base cost 1, tome menu/quickspell, `chargeCap = min(level+3, 10)`, recharge scaling, `LIGHT_READING`, `MagicImmune` gate), the three armor abilities (`Trinity` base 25 + per-effect costs, `PowerOfMany` ally empowerment, `AscendedForm` base 50 + 30-shield/10-turn `AscendBuff`), and the PRIEST/PALADIN subclasses | - | Not ported at all: the port's `T`-key heal (5+2/lvl through slow charges) is a stated stand-in with no Java equivalent, the crown offers no abilities (handled, not broken), no subclass choice is offered, and the talent tree is Mage's copy (already recorded on the talent row). Recorded by `MONSTER_ANALYSIS_CLERIC.md`. |
...
| Java block | TS destination | Status |
| --- | --- | --- |
| `Goo.act()`'s water-healing regen (`healInc` while submerged and below full HP) | `takeGooTurn` | **Now ported, found missing entirely while auditing the Sewers boss against `Goo.java`** - Goo's pump-up/slam mechanic was already live, but the separate "retreat into water to heal" behavior real players use as a tell had no equivalent at all. Simplified to a flat `+1 HP`/turn while standing in water and below full HP (real Java's `healInc` ramps `1`->`3` only under the `STRONGER_BOSSES` challenge, not modeled here) and does not interact with a `LockedFloor` door-lock countdown, since this port has no boss-room door-lock system at all - both narrower, honestly-flagged gaps rather than a rushed full match. **Corrected 2026-09-17, pump lifecycle (audited against `Goo.act()`/`getCloser()`/`getFurther()`):** any Goo step now discharges a pump-up in progress (both Java movers zero `pumpedUp`, then the move proceeds), and a Goo that cannot see the hero drops the pump on its turn (Java clears the charge outside HUNTING). |
...
| `Bat.baseSpeed = 2f` (double-speed turns) | - | Not ported: the MWL monster schema has no speed column and every mob acts 1:1 - recorded by `MONSTER_ANALYSIS_BAT_ALBINO_SWARM_SPINNER.md`. |
| `Albino.attackProc()`: half of landed hits with `damage > 0` apply `Bleeding.set(Random.NormalFloat(2, 3))` | `mobOnHit`'s albino branch | **Ported 2026-09-19**, correcting a wrong-shaped stand-in: the branch applied `poison` on a bare half chance (its comment claiming poison was the only available DoT primitive was stale - `setBleeding`, its tick, and its cure paths all exist). Now `setBleeding(defender, 2 + Random.float(1))` gated on `damage > 0`; the uniform `[2, 3)` draw is a stated stand-in for Java's bell-curved `NormalFloat(2, 3)`. |
| `Swarm.lootChance()`: `1/(6*(generation+1))` times the `(5-n)/5` LimitedDrops decay | shared `MOB_LOOT` roll site, `LIMITED_DROP_DECAY` | **Ported 2026-09-19**, closing a half-port: the decay half rode `LIMITED_DROP_DECAY` but split descendants rolled at the full generation-0 `1/6` (the `monsters.ts` comment already promised the divisor). The roll now divides by the creature's own `generation + 1`. |
| `Spinner` poison resistance (half duration) and `Web` immunity | `advanceBuffs`, web blob branch | Ported (2026-09-20) with a corrected mechanism on both halves: the resistance halves each poison *tick* with `Math.round` (the tick routes through `Char.damage()`, whose `resist()` halves the `Poison`-class damage - durations are untouched, so the old "no halving mechanism" note is retired), and `Web`-immune spinners walk through webs while occupied cells are consumed and rooted 5 (`Level.occupyCell()`, tag `v3.3.8` - the old "moot" note predates the consume-on-step shape). Pinned in `test:simulation`. Recorded by `MONSTER_ANALYSIS_BAT_ALBINO_SWARM_SPINNER.md`. |
| `Statue` PASSIVE/wake rules shared with `ArmoredStatue` (`damage()` flips to HUNTING, `add()` flips on NEGATIVE) | `takeStatueTurn`, the damage path's wake | **Ported 2026-09-19**, fixing a real inheritance miss: the armored kind matched neither the turn override (so it woke on sight and wandered as an ordinary mob) nor the damage wake (so a struck-but-unseen one slept forever). Both kinds now share the handler and wake on damage; the debuff wake also lands for both (the override returns before the generic sleeping branch). Simplification, stated on the method: woken statues never chase, attacking adjacent-only forever. |
| `Statue` weapon-driven combat (weapon damage/accuracy-factor/delay/reach/proc/enchantment, armor DR roll, Grim resistance) | - | Not ported: statues fight on the flat MWL stand-in line and the generated weapon/armor kit is loot only (dropped identified on death, matching `die()`). Recorded by `MONSTER_ANALYSIS_PIRANHA_STATUE_MIMIC_WRAITH.md`. |
| Base-`Mimic` chest disguise and NEUTRAL alignment (hidden flat damage, INFINITE-accuracy surprise, open-to-reveal) | - | Not ported: a base mimic spawns as a visible sleeping monster, so the whole hidden branch never fires. The crystal kind is fully covered (disguise, hasted reveal, steal, flee, escape). Recorded by `MONSTER_ANALYSIS_PIRANHA_STATUE_MIMIC_WRAITH.md`. |
| `Wraith` tomb/haunted-remains spawns (`Heap.open()` TOMB branch, haunted fallback curse) | `spawnWraithAt` | Not ported: no tomb heaps and no haunted trigger exist, so the spawner (delay, stats, restore all real) serves only the CorpseDust quest kind and regular wraiths never appear. Recorded by `MONSTER_ANALYSIS_PIRANHA_STATUE_MIMIC_WRAITH.md`. |
| Piranha kill tally (`Statistics.piranhasKilled`, the kill-count badge) | `src/content/badges.mwl` piranhas row, `kill()` | Ported (2026-09-20): every piranha death counts toward the `piranhas` counter and the badge fires at 6 (`Piranha.die()`, tag `v3.3.8` - any cause, beaching included). Meat loot, water rules, and the FishingSpear hook were already real; recorded by `MONSTER_ANALYSIS_PIRANHA_STATUE_MIMIC_WRAITH.md`. |
| `Mob.rollToDropLoot()`'s `hero.lvl > maxLvl + 2` early return | the shared loot-roll section | **Ported 2026-09-19**: overleveled heroes rolled full loot from every mob. One `overleveled` constant now gates the elemental/warlock/scorpio/succubus branches, the `MOB_LOOT` loop, the wealth bonus, and the greatCrab meat; die() drops and the port-invented guard key stay outside it, matching Java's method boundary. |
| Wealth bonus roll counts (BOSS 15, MINIBOSS 5, else 1) | `tryWealthBonusDrop` call site | **Ported 2026-09-19**: a hand list gave goo/dm200/dm201 five rolls while real minibosses rolled once. Now read off `BOSS_KINDS`/`MINIBOSS_KINDS`. |
| `Property.BOSS`/`Property.MINIBOSS` membership (eye/warlock/pylon; elemental/eye/warlock) | `actor-rules.mwl` flag sets | **Ported 2026-09-19, corrected the same day**: the first pass added holders Java never declares - Eye and Warlock declare neither property (Eye is DEMONIC-only, Warlock UNDEAD-only), Pylon declares MINIBOSS but not BOSS, and only `NewbornFireElemental` carries MINIBOSS among elementals. The additions over-looted all three onto the 15-roll (boss) and 5-roll (miniboss) wealth tiers where Java gives the ordinary single roll (`Mob.die()`: 15/5/else 1). The sets are now exactly Java's declarers (minus unported CrystalSpire/FungalCore/GnollGeomancer and CrystalGuardian/FungalSentry/GnollSapper). Caught by the suite's own MINIBOSS/BOSS disjointness check the first time `test:simulation` ran past its hunger abort. |
| `CausticSlime` loot (inherits Slime's weapon line with the shared `SLIME_WEP` counter, plus a gated GooBlob) | `loot-rules.mwl` caustic row, shared loot loop, new GooBlob branch | **Ported 2026-09-19**: the row carried invented `0.5` meat and no blob. Now matches slime's line with a `causticSlime -> slime` counter mapping. |
| `Elemental` ELECTRIC-style resistance halving (`Property.ELECTRIC` lightning family) | blob `applyDamage`, lightning wand loop | Ported (2026-09-20) for the two shapes that reach mobs: the `Electricity` blob seam and the `WandOfLightning` zap loop both halve with `Math.round` for the shock subtype only (`Char.damage()`, tag `v3.3.8`). Stated gaps: Shocking arcs/darts and the Potential talent have no mob-damage seam to halve through; the ACIDIC corrosion half in the same seam was corrected to `Math.round` the same pass (odd ticks round up). Recorded by `MONSTER_ANALYSIS_SLIME_CAUSTIC_DM100_ELEMENTAL.md`. |
| Summoned newborn elemental scaling (`15 * regionScale` HP and friends) | `castSummonElemental` | Ported (2026-09-20): `regionScale = max(2, 1 + floor(depth/5))` (sewers/prison share scale 2, then 3/4/5) with damage `5s..5+5s`, attack `5+5s`, defense `5s`, HT `15s` (`Elemental.setSummonedALly()`, tag `v3.3.8`); the row stays the never-summoned template and the `0..5` DR tuple was already Java’s. Recorded by `MONSTER_ANALYSIS_SLIME_CAUSTIC_DM100_ELEMENTAL.md`. |
| GreatCrab meat quantity 2 | greatCrab loot branch | Simplified: drops a single meat (one item per cell). Recorded by `MONSTER_ANALYSIS_SLIME_CAUSTIC_DM100_ELEMENTAL.md`. |
| `RotLasher.attackProc()` unconditional 2-turn Cripple | `mobOnHit` lasher branch | **Ported 2026-09-19**: the proc had no branch at all. Recorded by `MONSTER_ANALYSIS_DEMONSPAWNER_SENTRY_ROTHEART_ROTLASHER.md`. |
| `RotHeart.defenseProc()` ToxicGas `5 + 3 * openNearby` | `mobOnHit` heart branch | **Ported 2026-09-19**: struck hearts seeded no gas. Passable stands in for non-solid; wand/bomb hits bypassing the hook are a recorded residual of Java's source-independent `defenseProc`. |
| `RotHeart.die()` Rotberry.Seed drop (no seed on burn-destroy) | heart-death site | **Ported 2026-09-19**: no seed dropped. Excluded for the `fire` kill cause exactly the way Java's `destroy()` path skips death processing; the +2000 quest score stays unmodeled (no quest-score table). |
| Quest scores (`questScores[1]` heart +2000 / lasher-hit -100) | - | Not ported: no quest-score table exists. Recorded by `MONSTER_ANALYSIS_DEMONSPAWNER_SENTRY_ROTHEART_ROTLASHER.md`. |
| SentryRoom Sentry `damage()` no-op invulnerability | `buffBlocked`, blob `applyDamage`, bomb blast | Ported (2026-09-20): the turret refuses every buff at the shared boundary (`add()` returns false) and takes no blob/bomb damage (`damage()` no-op); melee and zaps already defeat themselves on its infinite evasion, the same shape as the sheep. Recorded by `MONSTER_ANALYSIS_DEMONSPAWNER_SENTRY_ROTHEART_ROTLASHER.md`. |
| `FungalSentry` entire (poison extender, gas/poison immunities, ballistics reach, 200 HP) | - | Not ported at all. Recorded by `MONSTER_ANALYSIS_DEMONSPAWNER_SENTRY_ROTHEART_ROTLASHER.md`, re-audited with exact numbers by `MONSTER_ANALYSIS_UNPORTED_QUEST_MOBS.md`. |
| The mining-quest actors entire (`CrystalGuardian`/`CrystalSpire`/`CrystalWisp`, `FungalSentry`/`FungalCore`, `GnollSapper`/`GnollGeomancer`/`GnollGuard` - stats, kits, partner links, telegraphed rockfalls, pickaxe-mining, `MINE_CRYSTAL`, `questScores[2]`) | - | Not ported at all: all eight spawn at weight 0 from quest rooms this port never generates, so random rosters are unaffected and the blacksmith boss-bonus branch stays dead-but-correct. Pinned by the suite (no MWL row may exist). Recorded by `MONSTER_ANALYSIS_UNPORTED_QUEST_MOBS.md`. |
| The remaining unported spawnables (`GnollExile`/`HermitCrab` 1/50 alt exclusives, `GoldenMimic`/`EbonyMimic` chest tiers, `PhantomPiranha` 1/50, `FungalSpinner` quest-branch spawn, `MobSpawner` respawn actor, `DelayedRockFall` telegraph) | - | Not ported at all: the port's gnolls/crabs never swap, locked chests spawn base mimics with no per-floor ebony roll, piranhas never roll phantom, the mining branch has no quest-type distinction, and there is no respawn system. The eight ported swaps, the four rare injections, and the chaos roll check out (38th matrix fixed chaos 1/51 -> 1/50). Pinned by the suite (no MWL row may exist). Recorded by `MONSTER_ANALYSIS_RARE_SPAWNS.md`. |
| RotLasher armor `[0,8]` vs Java's flat 0 (no `drRoll` override) | `rotLasher` MWL row | **Ported 2026-09-19**: the row's 8 was simply wrong - now 0. Recorded by `MONSTER_ANALYSIS_DEMONSPAWNER_SENTRY_ROTHEART_ROTLASHER.md`. |
| `FetidRat.defenseProc()` unconditional StenchGas-20 seed | `mobOnHit` fetid branch | **Ported 2026-09-19**: struck rats seeded no gas. Same hook and wand/bomb residual as the heart gas. |
| `YogDzewa$Larva` DEMONIC membership | `actor-rules.mwl` demonic set | **Ported 2026-09-19**: larvae were missing, hiding them from the prismatic-light blind and starflower smite readers. `BOSS_MINION` stays unmodeled (its only reader is the unported Duelist Challenge). |
| Honeypot shatter into Bee (pot-holder targeting, honeyed charm, strike-back aggro) | `useHoneypotFlow`/`shatterHoneypotFlow` in `src/items/honeypot.ts` (the file-size refactor's twenty-second extraction, 2026-09-19, behavior-identical - the scene keeps the one-line `useHoneypot` adapter plus a builder)/`takeBeeTurn`, `Creature.potPos`/`potHolderId` | **Ported 2026-09-19, 40th matrix (`MONSTER_ANALYSIS_HONEYPOT_BEE.md`)**: one port action covers SHATTER (own cell) and THROW (aimed cell, bomb-style gate, thrown range 6); the bee spawns hostile with its pot anchor and hunts holder-first/mobs-near-pot/hero-near-pot through its own override at `viewDistance` 4, stats pinned live at depths 1/5/14. Previously not ported (dead bee row). Residuals: stung mobs don't retaliate, no honeyed-charm path, no `ShatteredPot` item, pits refused outright. **2026-09-20:** the `chooseEnemy` cascade moved to `beeTarget` in `src/simulation/targeting.ts` as the file-size refactor's forty-first extraction, behavior-identical - holder-first at any range with the gone-holder fall-through to pot logic, the scene keeping the one-line adapter. |
| FetidRat `questScores[0] -= 50` on dry-hero ooze | - | Not ported: no quest-score table exists. Recorded by `MONSTER_ANALYSIS_FETIDRAT_BEE_LARVA.md`. |
...
| `Monk`/`Senior` Focus (`Monk.act()`'s `focusCooldown` decaying every one of the monk's own turns, re-attaching a dodge-the-next-hit buff once it reaches 0 while `HUNTING`) | `Creature.focusCooldown`, `tickMonsterTurnEnd` in `src/simulation/buffs.ts` (the turn-end buff ticks moved 2026-09-20 as the file-size refactor's thirty-fourth extraction, behavior-identical - death-mark/time-bubble stay in `afterMonsterTurn`), `moveTo`, `attack()`'s Focus-consuming dodge branch, the initial grant in `spawnMonster` | Ported: the floating 6–7-turn cooldown is reduced by one action-time unit after each Monk/Senior turn, plus Java's movement reductions (0.67 for Monk and an additional 1.66 for Senior), then reattaches the dodge buff while the mob sees the hero. Senior inherits the same parry reset and defense behavior. The shared buff map remains the carrier, so Java's sprite/audio parry presentation is not modeled. |
...
| `Eye.doAttack()`/`deathGaze()`: a two-turn ranged beam (charge, then fire a real magic-hit-roll 30-50 line attack bypassing armor/DR), with 1/4 damage taken while charged and a 4-6 turn cooldown after firing | `takeMonsterTurn`'s eye branch, `beamCharged`/`beamCooldown` fields on `Creature`, the `Eye.damage()` quarter-damage check folded into the shared attack damage-modifier chain, `simulation/turns.ts`'s `monsterTurnCost` hook (from the `firstSummon` fix) for the charge turn's real 2x cost | **Ported, replacing a completely wrong-shaped stand-in.** The previous implementation applied a 1.5x damage multiplier to every 3rd landed *melee* hit - not a simplification of the real ability, a different mechanic entirely: fetched `Eye.java` and confirmed DeathGaze is 100% ranged, never a melee proc at all (Eye's own melee `damageRoll()`/`attackSkill()` exist only as the generic fallback `canAttack` uses when the beam is on cooldown or blocked). Real Java: turn 1 charges (`spend(attackDelay()*2f)`, no damage, `((EyeSprite)sprite).charge()`), during which incoming damage is quartered (`if (beamCharged) dmg /= 4` in `Eye.damage()`); turn 2 fires along a `Ballistica` line, rolling a real magic hit (`hit(this, ch, true)`) and, on a hit, `Random.NormalIntRange(30,50)` damage applied via a direct `ch.damage()` call that never subtracts armor/DR at all (unlike this port's own `zapHero`, which does subtract armor for its bolt-throwing mob kinds - confirmed by reading the exact call chain, not assumed); then a `Random.IntRange(4,6)` cooldown. This port's version reuses the same `Roguelike.canTarget`/line-of-sight simplification already established for DM200's vent and Spinner's web instead of Java's full multi-target beam-with-terrain-destruction and BFS reachability check (the beam strikes every char along `traceLine(monster, hero).slice(1)` with the real magic hit and `NormalIntRange(30, 50)` per victim - hero, ally or enemy alike (ported 2026-09-17: hero keeps its armor-bypassing path, others take direct hp + floater + kill, plus the `Aggression` half-damage rule and the eye's own invisibility dispel; the line is drawn fresh to the hero's current cell rather than Java's stored `beamTarget`, with no fire-time recheck just as Java fires unconditionally once charged)). Browser-verified live: a fresh eye's first `takeMonsterTurn` call charged (`beamCharged: true`, `pendingMonsterTurnCost: 2`); a 20-damage test attack while charged did exactly `5` (floor(20/4)) versus the full `20` once un-charged; forcing a charged eye to fire dealt damage in the real 30-50 range and set a fresh cooldown in the real 4-6 range, with `beamCharged` correctly reset to `false`. |
| `RipperDemon.Hunting.act()`: a two-turn telegraphed pounce - arm the landing cell when off cooldown, enemy in FOV, unrooted and 3+ cells away (far-side prediction on a moved enemy, direct aim otherwise, then a same-cell fallback), and on the next turn reset the cooldown to `NormalIntRange(2, 4)`, re-trace the ray, bounce off an occupied landing to the nearest free neighbour (abort boxed in), and land an infinite-accuracy hit plus `Bleeding` at 0.75x a fresh damage roll | `simulation/ripperLeap.ts` (trigger gate, `CIRCLE8` landing prediction, cooldown roll, bounce search - all pinned by `verifyRipperLeap`'s 5 checks) plus `takeRipperLeapTrigger` (ranged table), `executeRipperLeap` (whole-turn overrides, so the pounce preempts even adjacent melee) and `resolveRipperPounce` (shared `runAttackResolution` force-hit path, `setBleeding`, `mobOnHit`) in `dungeonScene.ts`; `leapTarget`/`leapCooldown`/`leapLastEnemy`/`leapPrevEnemy` on `Creature`, persisted through save/load; `port.log.ripperleap` in all 19 locales, carrying SPD's own `ripperdemon.leap` wording per locale | **Ported 2026-09-17 as the fifth family matrix's pilot recommendation** (`MONSTER_ANALYSIS_SUCCUBUS_EYE_SCORPIO_RIPPER.md` called the Ripper a stat row with no behavior - the migration's first stateful movement ability). Real Java (`RipperDemon.java`, tag `v3.3.8`): `act()` decrements `leapCooldown` when unparalysed and records `enemy.pos` every turn except the wandering-to-hunting transition (this port's pre-turn hook rotates every turn, matching the steady state; the single skipped transition is not reproduced); the trigger spends `gate(TICK, enemy.cooldown(), 3*TICK)` (collapses to 1 here - the port hero acts on whole turns); the warning carries the red `TargetedCell` marker, the `leapPrep` crouch and `Dungeon.hero.interrupt()`, of which only the log line exists here (no cell-overlay primitive, synchronous hero turns - the same skip the eye charge documents); the sprite jump and push-aside resolve as an instant relocation; the leap ray is MWG `impassable`-stopped rather than solid-stopped, so a chasm-crossing leap truncates at the edge instead of clearing it. Live-verified (`tools/scratch/ripper-leap-livecheck.mjs`, 7/7: trigger arms the hero cell at cost 1, execution clears/resets 2-4/relocates/hurts/bleeds, rooted stands down with cooldown reset, hook ticks/freezes/rotates, no page errors - screenshot shows the French warning line, the 21-damage pounce and the `bleeding` status). |
| `Succubus.getCloser()`/`blink()`: while hunting, blink to a seen enemy more than 2 cells away when off cooldown and unrooted - a `PROJECTILE` ray to the collision cell (backing up one on an occupied landing, random free passable neighbour of the collision as fallback), arriving free with the cooldown reset to `IntRange(4, 6)`; the cooldown ticks only on ordinary-approach turns | `simulation/succubusBlink.ts` (attempt gate, cooldown roll, ray-backup plus neighbour-fallback destination search - all pinned by `verifySuccubusBlink`'s 5 checks) plus `trySuccubusBlink` ahead of the shared mover in `dungeonScene.ts` (relocation reuses the golem teleport's move-plus-`playTeleportAppear` pair, cost 0 through `pendingMonsterTurnCost`); `blinkCooldown` on `Creature`, persisted through save/load | **Ported 2026-09-17**, answering the fifth family matrix's open question for blink (the Barrier-overflow feed stays open - no generic ally Barrier pool exists). Real Java (`Succubus.java`, tag `v3.3.8`): the `getCloser` override attempts the blink exactly under this gate (a failed attempt wastes the turn, a landed one nets to zero via `spend(-1/speed())` against the act's own spend); `blink()` reroutes only off `avoid` cells (no avoid map exists here) and the `LARGE` open-space check (she is not large); the landing additionally requires a passable cell where Java only reroutes off `avoid` (unreachable in practice - opaque walls block the sight the attempt needs - so the guard only fires where Java would have embedded her); blink carries no log line on either side, and the aim stays hero-directed under charm like every other mover here. Live-verified (`tools/scratch/succubus-blink-livecheck.mjs`, 5/5: adjacent landing from 3+ with a 4-6 reset at zero cost, approach turns tick and fall through, rooted holds still while ticking, no page errors). |
...
| `DwarfKing` phase machine (P1 summons + LINK/TELE, P2 throne/shield/waves, P3 bleed/summons/viscosity) | `takeKingTurn` + `summonKingAdd`/`kingP1Summon`/`kingAbility`/`kingWave`, `kingPhase`/`kingShield`/cooldown fields, `attack()`'s shield/LifeLink/P1-accel hooks | Ported, checked against tag `v3.3.8` (`DwarfKing.java`) - replacing the old sketch outright, whose half-HP Fury has no Java basis at all and whose hold-the-barrier turn ran the real barrier backwards (it is a shield pool on the King, not inaction). P1 hunts with exact summon/ability cooldowns (10-14, damage-accelerated `-= taken/8`), real ghoul/monk/warlock/golem rotation, furthest-subject LINK (damage splits evenly onto the King through his own shield) and TELE (subject beside the hero, real yells); at HP<=50 (100 challenge) P2 goes immobile with a full-HP shield and the real wave schedule (ghoul/monk/warlock/golem counts and thresholds both modes, real wave yells, `KingDamager` HT/12 self-chip per batch); at shield 0, P3 summons under 4 adds and yells once under 20 HP. **P3 viscosity-deferral is now ported too** (2026-09-11): `DwarfKing.damage()`'s phase-3 branch banks every hit whose source is not its own payout into the shared `Viscosity.DeferedDamage` pool (`deferKingDamage`, checked at the central damage boundary before the LifeLink split so the King's share defers too) and pays it out on his own turns (`tickMonsterDeferredDamage`, `max(1, floor(pool*0.1))` after a one-turn delay) - the King therefore takes no direct HP damage in P3, exactly like Java. Remaining: the LloydsBeacon upgrade, and presentation (the King's Crown drop is now granted on his death - see `kill` **Corrected 2026-09-17, P3 reinforcement (audited against `DwarfKing.act()`):** Java gates P3 summons on fewer than 4 *pending* arrivals, an unbounded one-per-turn flood - the port capped live servants at 4 and never advanced the rotation, freezing P3 on one kind. P3 now reinforces every turn with no live cap and counts each success into the rotation. **P2 throne teleport and P3 bleed latch ported (2026-09-18, checked against `DwarfKing.java` tag `v3.3.8`):** the P1->P2 transition now moves the King onto `CITY_THRONE` through the shared appear presentation (Java's `ScrollOfTeleportation.appear(this, CityBossLevel.throne)`; the `IMMOVABLE` pin itself stays immobile-by-return - per-instance immovability has no kind-set seam); and P3 latches the bar-bleed flag (`bossBleedLatched`, re-derived from the persisted phase on load) instead of waiting for the 25% HP crossing.'s king branch). **2026-09-19, boss-transition matrix (`MONSTER_ANALYSIS_BOSS_TRANSITIONS_TENGU_KING_YOG.md`):** the transitions fired a turn late - `takeKingTurn`'s re-check was the only call site, so a crossing hit left the King un-transitioned until his next turn and a bomb/DoT/blob tick in between hit the old phase. New `kingDamageHook` fires the same table from the damage event (`attack()` tail, `applyBlastDamage`, the DoT tick, blob `applyDamage`), accel-first per Java's order, with the table's own `once` latch keeping the turn-start re-check safe; lethal P1 damage still kills outright, verified against `Char.java` (`die()` runs inside `super.damage()` before the phase branch). The P1 `-= taken/8` accel now also runs on the blast/DoT/blob seams, not just the attack tail. **Follow-up (same date):** the trap/blob/DoT seams bypassed the P2 shield absorb too (`ShieldBuff.processDamage()` runs inside Java's `Char.damage()` for every source), so a burning P2 King bled HP through a full shield - all five seams now share one pure `absorbShield` helper (`src/simulation/buffs.ts`, pinned in `verifyCombat.mjs`), and the trap path gained the hook/accel as well. Residual: the wand-zap path still bypasses the whole `damage()` chain (shield, LifeLink, curves), which needs routing work, not another copy of the block. **Bombs 2026-09-19 (`MONSTER_ANALYSIS_BOMBS_BLAST_SEAMS.md`):** the live bomb seam (`detonateBomb`'s local blast, not the scene's `applyBlastDamage`) bypassed all of it too - now shares `absorbShield` and fires `kingDamageHook` through a new injected context callback, accel-first. Six seams honor the shield and the transitions; wand-zaps remain the outlier. The same matrix verified every blast variant's numbers exact (including Tengu-thrown `5+d..10+2d` and the resisted holy 50%) and recorded two open residuals (bombs spare allies; shrapnel ignores line-of-sight). **34th matrix, 2026-09-19 (`MONSTER_ANALYSIS_DWARF_KING.md`):** full script re-audit - TELE moves the King himself first (past himself away from the hero, else the open neighbour furthest from the hero) instead of teleporting the servant alone, and the servant lands only on a strictly-closer cell; LifeLink is two-way `ceil(dmg/(links+1))` (King damage now splits onto every live subject; the old code halved add damage only), with P1 `-taken/8` accel on both shares; `lastAbility` persists even on whiffed rounds; P2 waves pace on the plan's `spend` cadence (3 for wave-1 schedules, 1 otherwise) instead of every turn; the P2 shield chips per dead P2-wave add (`kingDamager` flag, transition check attached) instead of per wave-turn, so P2 stalls until adds die; servants carry `noExp` (Java's arrival `maxLvl = -2`: no XP, no loot, all phases; the Imp token drop stays ungated); death gained the real `defeated` yell, the Degrade cleanse and the LloydsBeacon upgrade; LINK alternates the real `lifelink_1`/`lifelink_2` (retiring invented `port.log.kinglink`); the non-challenge wave-3 yell fires unconditionally (it was dead code). `kingWaveCd`/`noExp`/`kingDamager` persist in the save; wave plans plus cadence pinned in `test:simulation`. Residuals: pedestal arrivals with delay/damage; barrier +1/turn regen and shield/20 decay; the 100-turn link expiry; conversion chip/link-drop; bomb/DoT link splits; throne-heap spill; per-instance IMMOVABLE; `LockedFloor.addTime(20)`; `BOSS_MINION`; euclidean servant picks. |
...
was wrong, corrected 2026-09-15 - and that correction was itself wrong, corrected 2026-09-19: the "scaled stat line" (60 HP, 20/10, 6-12, 0-5) was never live either. `summonFist` overwrites the template with Java's verbatim line (300 HP, 36 accuracy, 20 evasion, 18-36 damage, 22-44 for rusted, 0-15 armor, EXP 25); only the abilities differ per subclass, and the `monsters.mwl` row is just the never-spawned template. Fists also keep Java's own `maxLvl = -2`, granting no experience like the King and
...
furrow rolls that upgrade a plain GRASS neighbour, then plain grass across its 3x3 - weights corrected 2026-09-19, the old `chance(2/3) ? 2 : 1` dealt 2 cells with 2/3 where Java deals 1 with 2/3), `SoiledFist.zap()` roots its
target only on a landed hit and grows grass (1-in-5 tall) across the target's 3x3 hit or miss (corrected 2026-09-19 - roots were unconditional and the whole zap was accuracy-gated), both gated by `canSpreadGrass()` (more than 4 cells from
...
Blindness (1.5x on the pin, 3x on death) and Dark detaches the hero's Light - corrected 2026-09-19, Dark paid Bright's daze on both edges and now pays nothing (no Light model). Java's own Blindness is only a cosmetic
screen darkening (a `FlavourBuff` with no mechanical effect in v3.3.8), and this port has no Light artifact to
weaken/detach, so Bright's feedback keeps the port's `daze` stand-in. **The arena seal is now ported too**
...
other. **Corrected 2026-09-17, Yog's own turn (audited against `YogDzewa.act()` line by line): six gaps, all in `takeYogTurn`.** (1) Beams were held while fists lived - Java aims and fires every turn regardless of fists (only damage is gated by invulnerability); the fist early-return is gone. (2) Firing ended the turn - Java re-aims the same turn when the cooldown allows; the fire block now falls through to the aim block. (3) Summons ran one per turn with a floor of 5 - Java loops `while (summonCooldown <= 0)` with no floor (phase 5 opens at -15 for a burst) plus `MIN_SUMMON_CD - phase + 1` while a fist lives; now matched. (4) No phase-5 beam clamp - leftover cooldowns now cap at 2. (5) Aiming needed range 8 and line - Java aims map-wide with `WONT_STOP` rays and no gate; the gate is gone. (6) Firing never broke invisibility - `act()` dispels on every firing turn, victims or not; now dispelled up front, same as the Eye's own gaze. **Closed 2026-09-18:** the aiming turn now spends Java's own `GameMath.gate(TICK, ceil(hero.cooldown()), 3*TICK)` through `pendingMonsterTurnCost` (the same gated expression the newborn telegraph uses) and drops auto-travel as the `Dungeon.hero.interrupt()` stand-in (the same one the talisman site uses). The fistless-P5 "bleed" was never damage over time - it is `BossHealthBar.bleed(true)` in `processFistDeath`, now latched (`bossBleedLatched`, re-derived from the persisted phase on load) exactly like the King's P3 flag. Pinned by a `verifyVault.mjs` check and browser-verified live (`tools/scratch/halls-centerpiece-livecheck.mjs`,
5/5: the frames, their origins, the blank rest of the floor, and the two layers straddling the wall layer). **2026-09-19, boss-transition matrix (`MONSTER_ANALYSIS_BOSS_TRANSITIONS_TENGU_KING_YOG.md`):** two more `YogDzewa.damage()` halves were missing - taken damage (measured post-clamp) now accelerates both cooldowns (`-= dmgTaken/10`), and a gate crossing floors both at 5 (`addFist()`'s own pair). The same pass fixed Yog-death cleanup omitting `'larva'` (Java kills Larva with the other minions) and verified the fist guard already holds (no gate can advance under a live fist). **36th matrix, 2026-09-19 (`MONSTER_ANALYSIS_YOGFISTS.md`):** full fist-kit re-audit. Elemental zaps now cool down (`NormalFloat(8, 12)` float `fistZapCd`, persisted, bright/dark exempt - a cooling fist steps closer instead of zapping) and only soiled/bright/dark roll to hit (burning/rotting/rusted land unconditionally); burning zaps reignite plus top up fire 3x3 with no direct damage, evaporating water instead, with the per-turn evaporation act; rotting hits convert to 60% Bleeding on both damage paths (harvest strikes exempt, Corrosion halved first, water heal `HT/50` on fist turns); the five invented contact riders are gone (rotting ooze only); dark no longer pays bright's daze prices; bright's invented frost immunity is deleted (immunities pinned in `test:simulation`); fist rows carry Java's EXP 25. Residuals: in-melee elemental zaps, 6-cell (not sight-limited) zap range, unmodeled FIERY/ELECTRIC damage halving, presentation (zap sprites, steam/leaf, flashes, teleport yell, "Bleeding N" text, kill lines), Dark's Light halves, locked-floor time, chasm fire seeding. |
| `Goo`: full kit (see before) | `MONSTERS.goo`, `liveStats`, `takeGooTurn` | Ported, with one simplification: the pumped slam only fires at melee range, not as the 2-tile lunge. **The arena seal is now ported too** (2026-09-14, `SewerBossLevel.java` 177-195): Goo spawns SLEEPING - Java's `Mob.state` default, which the port's boss-wide awake exemption had wrongly lifted - wakes on the real detection roll, and `takeGooTurn` seals on his first acting turn (Java's `act()` site; the `damage()`/`notice()` sites reduce to the same turn via the existing hurt wake, at most one round late, stated). The entrance becomes live + paint `WATER` with restitched frames and a refreshed water layer, and the flag persists through save/load with the tile re-applied. Verified live (`tools/scratch/sewer-seal-livecheck.mjs`, 6/6 assertions: asleep on entry, wake spends the turn unsealed, first acting turn drowns the entrance, round-trip keeps flag/tile/Goo). **The `unseal()` half is now ported too (2026-09-16)** - see the auto-descent note in the Sewer-boss section below. Not ported, stated: the `LockedFloor` buff that actually bars Java's exit (this port's exit is the stairs, gated on the boss's death the same way - and Java's own `WATER` is walkable too, so the tile never blocked anyone), the `SEWERS_BOSS` switch (already playing on boss-floor entry), the ripple (no one-shot particle hook), and Goo's `notice` yell. The boss-challenge-badge flag is ported - see the `BOSS_CHALLENGE_1..5` row. **41st matrix, 2026-09-20 (`Goo.java`, tag `v3.3.8`):** `attackProc` 1-in-3 Ooze (`Ooze.DURATION`) now lands through the shared buff path; the `Goo.damage()` pump-shake claim is removed - no such code exists, the only pump shake is the dead-on-arrival `attackProc` one (`damageRoll()` already consumed the pump) and Java never interrupts the charge on damage; `STRONGER_BOSSES` HP 120 wired in `monsterSpawnProfile`; water-heal and pumped-slam fouls wired via `foulBossChallenge` (pinned in `verifySimulation.mjs`: one foul per slam). `Char.Property.ACIDIC` halves Corrosion damage (`Char.java`, resistances list): Goo, CausticSlime and the acidic mob halve each tick, the rotting fist keeps its convert-instead branch, and the Ooze-buff half of ACIDIC already lived in the immunity table. Stated residuals: the `bossScores` +/-100/1000 economy (score here is depth/level/gold in `rankings.ts`), `LockedFloor` add/removeTime (buff unmodeled), the `attackProc` burst and the `act()` non-HUNTING pump clear. **2026-09-20:** pump discharge on steps is now complete - previously only the ratmogrified branch cleared `pumped` on a step, so a charging Goo that the hero stepped away from kept its charge while closing in. All ten voluntary-step sites (hunting, wandering/lastSeen, patrol, ally-pursuit, amok, aggression, terror-flee, ratmogrified, stepAway, fleeCrystalMimic) now route through a shared `stepMonster` helper that clears the pump before `moveTo`, matching `Goo.getCloser()`/`getFurther()` clearing on entry; forced relocations (knockback, teleports, leaps, blinks, the golem self-teleport) stay on `moveTo` directly, since Java never routes those through getCloser/getFurther. |
...
| `equipRing`/`equipArmor`/`equipWeapon`'s unconditional `identified: true` when returning the outgoing piece to the bag | `src/items/equipment.ts`, `EquippedRing.identified`, `weaponIdentified`/`armorIdentified` | **Real bug found and fixed, 2026-09-19**, while re-checking the stale comment this ROADMAP line's own text carried ("equipped fields... always fully populated - swapped-out gear even returns `identified: true`" - offered as the reason Java's `onTalentUpgraded` intuition-identify had "no observable target"). That claim was itself the bug: `Item.identify()` is never implied by merely equipping something in Java - only Veteran's/Thief's/Adventurer's Intuition rank 2 (already ported, gated on equip) or an outside identify makes it so - but this port's three equip functions hardcoded `identified: true` on every piece returned to the bag on a swap, silently fully identifying any worn gear the instant it was taken off, talent or not. Fixed by tracking the real identified state through the equip cycle: `EquippedRing` gained an `identified` field (rings already had no live-worn state to read it from); weapon/armor gained `weaponIdentified`/`armorIdentified` scene fields (defaulting `true`, matching Java's always-known starting gear), read from the incoming item's real `identified` flag at equip time and written back unmodified at unequip time, instead of a hardcoded `true`. Browser-verified live: an unidentified ring/weapon equipped and then swapped out returned to the bag still unidentified in both cases. **Closed the same day**: the narrower gap this line's text also named - Java's `onTalentUpgraded` identifying *already-equipped* gear the instant a hero reaches intuition rank 2, rather than waiting for the next equip - is now wired too (`identifyOnTalentUpgraded`, called from the talent-spend site right alongside the rank increment): reaching rank 2 in Veteran's/Thief's/Adventurer's Intuition identifies the currently-equipped armor/ring/weapon immediately if it wasn't already, and (matching this port's own `procIdentifyTalents` convention for "any newly-identified item") also fires Test Subject/Tested Hypothesis. Rank 1's Thief's Intuition `setKnown()` stays unported for the same binary-identified-model reason as before. Browser-verified live: equipping an unidentified armor as Warrior left it unidentified, and reaching Veteran's Intuition rank 2 immediately flipped it to identified without a re-equip. |
| `EMPOWERING_SCROLLS` / `ENHANCED_RINGS` / `LIGHT_CLOAK` / `ALLY_WARP` / `SEER_SHOT` (the five class-wide tier-3 talents whose mechanics were listed as missing) | `talentEffects.ts` (five formulas), `dungeonScene.ts` (`empoweredZaps`/`effectiveZapLevel`, `enhancedRingsTurns`/`effectiveRing`, `lightCloakChargeMultiplier`, `tryAllyWarp`, `procSeerShot`/`seerCells`), save/load, `verifySimulation.mjs` | **Ported (2026-09-19).** `EMPOWERING_SCROLLS`: any scroll read arms 1/2/3 +3-level zap charges (Mage; transmutation arms at its own consume point, cancelled pickers arm nothing) and every zap resolves through `effectiveZapLevel()` - damage bounds, corrosion seed/strength, chill duration, blast-wave push, prismatic daze, excess-charge shield, the regrowth/fireblast/transfusion helpers, and disintegration's targeting range (previewed prospectively, consumed once on fire). `ENHANCED_RINGS`: any artifact use arms 3/6/9 turns of +1 ring upgrade (Rogue), read through `effectiveRing()` at every one of the ~40 formula sites while equip/cleanse/transmute/save keep the raw ring; armed on the use attempt (stated: per-artifact success is not reported back through the dispatch). `LIGHT_CLOAK`: the cross-hero 7/13/20% artifact-charge bonus is folded exactly into every passive gain (cloak/armor/spellbook directly, toolkit/armband/horn/sandals/talisman/rose through their existing energy-ring multiplier args - multiplication commutes, so the fold is exact); the Rogue unequipped-use half is satisfied by construction (a carried cloak is always usable at the full equipped rate) - **Divergence (deliberate)**: Java refuses the unequipped use entirely instead. The cloak `talentScale` clause in `artifactRecharge.ts` stays unapplied for the same reason (no equip slot to be unequipped from). `ALLY_WARP`: a bumped non-immovable ally swaps places with the Mage at 2/4/6 range with the shared teleport presentation, free like every other ally order (stated: no tap-creature seam exists, so the bump is the tap). `SEER_SHOT`: thrown and bow attacks reveal the landing cell's 3x3 (permanent explored marking plus a timed creature-visibility layer, 5/10/15 turns, flat 20-turn cooldown, floor-indexed cells cleared on descent) - vision, not search, so no secret discovery (stated: Java fires at aimed ground, but targeting here is creature-based). All four state fields persist; formulas pinned in `test:simulation` (15 asserts). |
| `Talent.initClassTalents()`'s own tier-3 pair (`HOLD_FAST`/`STRONGMAN`, `EMPOWERING_SCROLLS`/`ALLY_WARP`, `ENHANCED_RINGS`/`LIGHT_CLOAK`, `POINT_BLANK`/`SEER_SHOT`, `PRECISE_ASSAULT`/`DEADLY_FOLLOWUP`) - a genuinely separate call from `initSubclassTalents()`'s own three, both writing into the same tier-3 map | `talent-rules.mwl`'s `talentSubclassEntries` table, `syncHeroFromStats`, `useWeaponAbility`/`armPreciseAssault`, `heroOnHit` | **Found and fixed, 2026-09-18.** A real Java class-wide tier-3 pair, on top of every subclass's own three, was missing from all 10 subclass rows in this port's talent table - every subclass offered only 3 tier-3 talents where Java's own `Hero.talents.get(2)` accumulates 5 (`initClassTalents`'s fixed pair for the *class*, `initSubclassTalents`'s three for the *subclass*, both `.put()`-ing into the same map). This was flagged as a real suspect by an earlier audit (see the `seed=123456789/depth=1` connector-count investigation above, "or something in `createBranches`'s failure/retry bookkeeping") without being confirmed as a talent-table gap - found instead while chasing the section-9 RNG-trace tool's exact divergence draw index for an unrelated levelgen investigation surfaced `PRECISE_ASSAULT`'s accuracy formula already written and dead (`this.talentRank('point_blank')` in `adjacentAccFactor` and `this.talentRank('strongman')` in the STR formula were *also* both already-written, correctly-shaped, permanently-inert code - the same "formula ported ahead of the table row" pattern three separate times). All 10 talents carry real, complete, pre-translated catalogue strings in `spdMessages.ts`, confirming they are genuine Java talents rather than invented ones. Fixed: each subclass row now lists its class's fixed pair first, then its own three. `STRONGMAN` (`floor(STR*(0.03+0.05*points))` STR bonus) and `POINT_BLANK` (accuracy factor in `adjacentAccFactor`) needed no further code at all - only the table entry, since both formulas already existed unreachable. Three needed new wiring: **`HOLD_FAST`** - `HoldFast.armorBonus()`'s `NormalIntRange(0, 2*points)` roll while the hero hasn't moved since last waiting, folded additively onto `hero.armor[1]` (this port's one `normalRange(armor[0],armor[1])` roll has no seam for Java's second, independent roll - the same simplification `bark` already uses), tracked via `holdFastX`/`holdFastY` set on `wait` and compared against the hero's live position rather than a real detaching buff object (a hero who waits, walks off, then walks back onto the same cell without waiting again re-lights it, where Java's buff would already have detached - stated). **`PRECISE_ASSAULT`** - `armPreciseAssault()` arms `preciseAssaultReady` at every one of `useWeaponAbility`'s 7 completion sites, called *after* `attack()` for the one damage-strike case (matching Java's own `afterAbilityUsed` position at the end of the attack callback, which is what naturally keeps the triggering strike from boosting itself - no separate "not this weapon" marker was needed to reproduce that exclusion); consumed in `resolveHeroAbilityAttack` as `2^points` ACC, skipped (not consumed) while a Flail spin is active, matching Java's own carve-out. **`DEADLY_FOLLOWUP`** - marked on a thrown hit (excluding `SpiritArrow`, structurally unreachable for a Duelist), consumed on the next melee hit against the same target as `round(dmg*(1+0.08*points))`, last in the damage-bonus chain matching Java's own `onAttackProc` order; the existing `attacker === this.hero` reference-identity check (throws attack with a spread copy, never the live reference) already excludes a thrown hit from consuming its own mark, for free, matching Java's `attackingWeapon() instanceof MissileWeapon` exclusion without an extra flag. `ENHANCED_RINGS`, `LIGHT_CLOAK`, `EMPOWERING_SCROLLS`, `ALLY_WARP` and `SEER_SHOT` remain genuinely unwired past the table entry (each needs new mechanics: a ring-effective-level buff keyed off artifact use, a Cloak-of-Shadows charge-rate hook usable unequipped, a wand-zap level bonus keyed off scroll reads, an ally-swap-at-range interact, and an empty-cell-targeted map reveal this port's creature-only targeting has no seam for) - tracked as the remainder of ROADMAP.md's talent-formula item, not silently dropped. Browser-verified live via `window.__MWG__.currentScene`: `strongman` rank 3 raised STR from 10 to 11 exactly; `hold_fast` rank 3 raised `armor[1]` from 2 to 8 only while `holdFastX/Y` matched the hero's position; `precise_assault` rank 2 consumed `preciseAssaultReady` on a normal attack but left it untouched through a Flail-spin attack; `deadly_followup` rank 3 turned a fixed 10-damage hit into 12 (`round(10*1.24)`) only when the tracker matched the target, consuming it after. |
...
| `ClassArmor.charge` + `ClassArmor.Charger.act()` (`100/500f` per tick, `RingOfEnergy.armorChargeMultiplier`) | `armorCharge`, `recoverArmorCharge` in `spendHeroTurn`'s effect list | Ported, with one stated shape difference: Java keeps the charge on the class-armor **item**, while this port keeps it on the hero even though the class-armor item now exists (2026-09-19 row below) - moving the charge onto the worn id would strand it every time the armor is swapped out and back, and the hero-side pool is spent by whichever armor is worn. The rate, the 0-100 range, the 50 start and the Ring of Energy multiplier are Java's; because `finishHeroTurn` hands the clock one whole action, the tick is scaled by the action's own turn cost (a three-turn Endure regenerates three ticks, as Java's Charger actor does while the hero is busy). `Regeneration.regenOn()`'s starvation gate has no state to read here. |
...
| `HeroicLeap.activate()` - `Ballistica.STOP_SOLID\|STOP_TARGET` route, the one-cell walk-back off an occupied landing cell, `BODY_SLAM`'s `NormalIntRange(points, 4*points) + round(drRoll*0.25*points) - target.drRoll()`, `IMPACT_WAVE`'s `1+points` blast-wave shove with `Int(4) < points` Vulnerable, `DOUBLE_JUMP`'s tracker | `activateHeroicLeap`, `simulation/warriorAbilities.ts` | Ported. Browser-verified live: leaps stopped one cell short of an occupied cell, the neighbour took the body-slam roll, was shoved exactly `1 + points` cells, and took Vulnerable on the talent's own roll; charge went 100 -> 65.2 (35 spent, 0.2 of regen from the spent turn). The shove reuses the port's established straight forced-movement primitive (the same substitution the Blast Wave wand already documents) rather than Java's `throwChar` trajectory, so terrain-pressing and collision damage remain simplified. `hero.rooted`'s shake-and-return spends neither charge nor a turn, as Java. **2026-09-19 (25th matrix):** the shove and the `Int(4)` roll are now unconditional per neighbouring non-ally (both were gated on `impactWave > 0 && neighbour.hp > 0`, so untalented leaps never shoved and killing slams skipped a draw Java burns). Corpses stay put - Java's post-kill `throwChar` only slides an already-removed actor whose loot already dropped, which nothing observes (and this port's `moveTo` would risk trap triggers Java never produces). `Buff.prolong`'s extend (vs this port's set-to-5 `addBuff`) on a re-leap Vulnerable stays a stated gap. Live re-verification of an untalented leap's one-cell shove is owed. |
| `Shockwave.activate()` - `ConeAOE(aim, min(aim.dist, 5+EXPANDING_WAVE), 60+15*EXPANDING_WAVE, STOP_SOLID\|STOP_TARGET)`, `heroDamageIntRange(5+STR-10, 10+2*(STR-10)) * (1+0.2*SHOCK_FORCE) - drRoll`, `STRIKING_WAVE`'s `Int(10) < 3*points` proc-and-combo promotion, `SHOCK_FORCE`'s `Int(4) < points` paralysis else 5-turn cripple | `activateShockwave`, `simulation/warriorAbilities.ts` | Ported, reusing this port's line-for-line `ConeAOE` translation (`src/mechanics/cone.ts`) with Java's own true-distance clamp. Browser-verified live: a target 6 cells out was untouched at `EXPANDING_WAVE` 0 and hit for 9 + cripple at rank 4 (max distance 9), a target at exactly the 9-cell boundary was hit only with the talent, a second creature directly behind the first in the same line was shielded by it (Java's own `STOP_TARGET` ray stop - the leading creature ends the ray), something outside the 60-degree arc was untouched, the hero was never caught by his own cone, and live charge spend matched the HEROIC_ENERGY rank restored from a save. `hero.attackProc`'s proc half is this port's `heroOnHit` chain at the same point, and the Gladiator combo increment mirrors `attack()`'s own. `Talent.StrikingWaveTracker` (attached at rank 4) is not modelled - and it does have a reader (`Weapon.java` accuracy `multi += 0.2`), so rank 4 currently buys only the 100% proc rate here. **2026-09-19 (25th matrix) residuals:** striking-wave procs also drop `attackProc`'s damage reassignment (`PROVOKED_ANGER`/`LINGERING_MAGIC`/`SUCKER_PUNCH`/Battlemage `EMPOWERED_STRIKE`) - the void `heroOnHit` chain covers side effects only, and threading damage back is shared `heroOnHit`-contract work; neutral NPCs are spared by the standing centralized ability-damage convention while Java hits by alignment (`ch.alignment != hero.alignment`), which only true NPCs can observe since neutral Mimics are already included on both sides. |
| `Endure.activate()` + `EndureTracker` (`adjustDamageTaken`, `endEnduring`, `damageFactor`) | `activateEndure`, `endureAdjustDamageTaken`, `settleEndure`, `consumeEndureBonus`, `tickEndureDuration`, `simulation/warriorAbilities.ts` | Ported: 12 tracker turns, `hero.spendAndNext(3f)` (the only multi-turn ability), half damage while enduring with `SHRUG_IT_OFF`'s further `0.8^points` (60/68/74/80%), half of the pre-reduction damage banked, then at the hero's next action `SUSTAINED_RETRIBUTION`'s `1+0.15*points`, `EVEN_THE_ODDS`'s `1+0.05*points` per hostile within distance 2, and an even split over `1 + SUSTAINED_RETRIBUTION` strikes. **Two stated placements differ.** (1) Java applies the reduction pre-armor for a char attacker and post-armor for other sources; this port has one hero-damage boundary (`absorbHeroDamage`) and it sits post-armor, so the halving is slightly kinder to the hero than Java's melee case. (2) Java adds the banked counter-attack pre-armor; this port adds it where `damage` is already net of armor, so it lands a little harder. `settleEndure` runs at the start of the hero's *next* action (Java's `Hero.act()` site), not at the end of the casting turn, which would close the window before a blow landed. `Combo.addTime(3f)` is not modelled: this port's Gladiator combo is a bare landed-hit counter with no duration to extend. Browser-verified live end-to-end: cast spent 50 charge and three turns (77.5 -> 50.75 through the aim path, 100 -> 50.6 on a direct cast), a 40-damage hit returned 20 and banked 20, the settle produced one 20-damage strike, and the next melee attack dealt exactly 20 + 20. **2026-09-19 (25th matrix):** `damageBonus` int semantics fixed - banking truncates per hit, both ending `*=` truncate, the split is integer division, and a post-split zero detaches instead of arming phantom hits (pinned: bank 15 -> 7; 7 at rank 2 -> 3 over 3 hits; 2 at rank 3 -> detach; `settleEndure` already clears all fields on the `{0,0}` return). BODY_SLAM's hero-armor term inherits the port's established single-roll armor-tuple model (Java composes up to three `drRoll()` rolls) - the pure function is exact given its input, so rebuilding hero defense stays pipeline work. |
| The remaining two abilities: Mage `ElementalBlast`/`WildMagic` | - | **Not ported**, deliberately, and not offered: each needs systems this port does not have yet. Their data rows exist - names, charge costs, targeting modes and T4 talent lists - so the ported/non-ported split is one set in `armorAbilities.ts`, and the choice panel offers only what can actually run. **This row previously also listed Huntress `SpiritHawk`, Duelist `Feint`, `Challenge` and `ElementalStrike`, and Rogue `ShadowClone` as not ported - all five are ported now (see their own rows); the `SpiritHawk` half of that claim was already stale before its pass, not just made stale by it.** **2026-09-19:** `ElementalBlast`'s pure arithmetic is ported and pinned (`simulation/mageAbilities.ts`, `tools/verifyArmorAbilities.mjs` inside `test:simulation`) - the per-wand factors, `ELEMENTAL_POWER`/`BLAST_RADIUS`, the roomiest-cardinal aim, damage, the Transfusion undead/ally splits, Corrosion, nine buff durations, the Regrowth grass chance, knockback and `REACTIVE_BARRIER` - but the ability stays unoffered. `WildMagic` is untouched. **Root cause narrowed the same day, checked directly against both classes' real Java source (not the vaguer "staff-imbue system"/"wand-randomization pass" phrasing this row carried before): this port's hero carries at most one `wand` bag entry (`dungeonScene.ts`'s single `wandType: WandType` scalar, `equipWand`'s one stackable `id: 'wand'` slot), with no per-instance multiple-distinct-wands model at all. `MagesStaff.imbueWand()` (`items/weapon/melee/MagesStaff.java`, tag `v3.3.8`) lets the Mage attach *one of several carried wands* to the staff via `WndBag.ItemSelector`, syncing levels and merging charges between the two physical items; `WildMagic.activate()` (`actors/hero/abilities/mage/WildMagic.java`) literally calls `hero.belongings.getAllItems(Wand.class)`, shuffles every distinct wand the hero owns, and fires up to `4 + FIRE_EVERYTHING` of them at once. Neither ability is offerable without a real multiple-wands-carried inventory model first - and that model's blast radius reaches well past these two abilities (the Magical Holster bag row, Wand Preservation, and every wand-recharge site already assume exactly one wand), so this is not a two-ability feature gap but a standing architectural simplification with two visible symptoms. |
| `ElementalStrike.activate()` + `preAttackEffect`/`perCellEffect`/`perCharEffect` + `DirectedPowerTracker`/`ElementalStrikeLuckyTracker`/`ElementalStrikeFurrowCounter` (tag `v3.3.8`) | `activateElementalStrike`, `heroWeaponRoll`, `detonateConjuredBlast`, twenty helpers in `simulation/duelistAbilities.ts`, `luckyTracker` buff row, `elementalFurrow` save field | Ported (2026-09-19). The `WONT_STOP` aim, `4 + ELEMENTAL_REACH` clamp, `65 + 10*reach` cone, `1 + 0.30*STRIKING_FORCE` power multiplier, `0.30*targetsHit*points` DirectedPower boost on the forced primary swing, and all twenty-one imbuement branches (thirteen enchantments, eight curses, unenchanted `6-12`) reproduce Java's numbers - Kinetic splash `40%`, Blocking `6`/Vampiric `2.5`/Sacrificial `10`-self/`12`-others, blob/grass budget `8`, roots `6`, Elastic `5`, Lucky `12.5%`, Projecting `30%`, Corrupting `5-25%`, Grim `6-30%`, shared curse `50%`, Annoying `20%`, Polarized `24-36`. **Stated reductions**: no Freezing blob (Chilling clears fire and chills occupants, the frost-potion halves); no `HUNTING -> WANDERING` calm on Displacing (no mob-state field); Elastic sorts furthest-first per Java's own comment (its comparator sorts closest-first - taken as the bug) with no collision damage; Lucky-strike loot is a plain consumable, not `genConsumableDrop(-5)`; the tracker is a 9999-turn buff standing in for Java's permanent one; neutral NPCs are spared by the standing ability-damage filter; cone cast visuals/sounds have no seam. **2026-09-19:** the Antimagic zero-out is now routed too (`Char.damage()`'s `isImmune(srcClass)` gate against `AntiMagic.RESISTS`, tag `v3.3.8`): the base strike and Polarized pass `ElementalStrike.this` and the execute passes `Grim.class`, so all three deal a magic-immune victim nothing (`elementalStrikeResisted`, pinned in `test:simulation`); Kinetic/Projecting splashes pass their unresisted enchantment and the ConjuredBomb blast passes the unresisted base bomb, so those three are unaffected. Browser verification owed per section 10 (an Antimagic champion shrugging off the strike in a live run). |
| `Feint.activate()` + `AfterImage`/`FeintConfusion` (tag `v3.3.8`) | `activateFeint`, `spawnAfterImage`, the `attack()` gate on `defender.allyKind === 'afterImage'`, `takeAllyTurn`'s `afterImage` branch | Ported (2026-09-16). The hero dashes to an adjacent free cell (Java's own `too_far`/`bad_location`/rooted refusals, none of which spend charge or a turn) and leaves a decoy at the vacated cell; any attack against the decoy is intercepted before the hit roll (`defender.allyKind === 'afterImage'`, right where `SpiritHawk`'s own dodge gate sits) rather than actually resolving one, since Java's `defenseSkill()` side effect fires on every attempt regardless of hit or miss - the decoy never takes damage either way, so the observable result is identical. The interception applies `FeintConfusion` to the attacker (wasting its whole next turn, `Mob.act()`'s own early return, mirrored onto this port's existing `paralysis`/`frost` turn-skip gate in `takeMonsterTurn`) and, per talent, `FEIGNED_RETREAT`'s `2*points` hero Haste and `EXPOSE_WEAKNESS`'s `2*points` Vulnerable+Weakness on the attacker. The decoy itself self-destructs on its own first scheduled turn (`takeAllyTurn`'s `afterImage` branch), this port's equivalent of Java's `actPriority = HERO_PRIO+1` fading it just before the hero's next action. **One buff-duration wrinkle, not a behavior difference**: Java arms `FeintConfusion` for a bare "1" turn, but this port's `advanceBuffs` decrements before the skip-turn gate reads the value on the attacker's own next turn - a duration of 1 would already be gone by then, so the authored table uses 2 (documented in `buff-rules.mwl`) to make the buff still read truthy exactly once, matching Java's single lost turn (browser-verified: the buffed rat skipped its very next action and hit normally the turn after). **Two real gaps, both stated.** (1) `enemy.clearEnemy()` has no analog - this port's mobs hold no persistent enemy pointer to drop, since target is recomputed from field of view every turn, so a mob that still sees the hero in plain sight keeps hunting the hero instead of being forced onto the decoy; only a mob that cannot currently see the hero (already routed to the nearest visible ally) or one already adjacent to the decoy takes the bait. (2) `COUNTER_ABILITY`'s tracker (`Talent.CounterAbilityTacker`, a bare 3-turn flavour buff) is armed exactly as Java does, and its consumer is live too since 2026-09-17 (this row's "no counterpart" clause was stale): `useWeaponAbility` refunds `rank*0.375` through `gainWeaponCharge` after the spend and detaches the tracker with it (a refund, not the discount the stale catalogue desc states), which is `afterAbilityUsed`'s whole `COUNTER_ABILITY` job. Browser-verified live: casting adjacent to an awake, sleeping-false rat left the decoy at the hero's old cell and moved the hero to the aimed cell; the rat's own turn attacked the decoy (`"Rat marsupial manque vous (image)"` in the log) and came away with `feintConfusion: 2, vulnerable: 4, weakness: 4` while the hero held `haste: 6, counterAbility: 3` (ranks 3/2/1); the rat's very next turn did nothing (hero HP unchanged), and the turn after that it landed a normal hit. **30th matrix 2026-09-19 (`MONSTER_ANALYSIS_FEINT.md`): one real defect fixed - the decoy had none of Java's immunity surface (`add(Buff)` false plus the whole `BlobImmunity` set), so fire could ignite it and frost could freeze it past its destroy turn into permanence; buffs are now refused centrally in `buffBlocked` and toxic/corrosive/electricity direct damage skip it by name, all pinned in `test:simulation`. Dash, interception, talents, and one-turn life verified unchanged. Residuals: the forced retarget and displacement immunity stay unmodeled (no per-mob target override, kind-keyed immovable sets). |
| `ClassArmor` as an item: `AC_TRANSFER`, the armor-to-class-armor conversion of a worn armor, `updateArmor()`'s class-armor art | `CLASS_ARMOR_ID_BY_CLASS`/`isClassArmorId` (`src/items/catalog.ts`), `wearClassArmor`/`chooseArmorAbility`/`grantRatmogrify`, `transferClassArmor` (`src/items/equipment.ts`), the six `ITEM_KEYS` entries (`src/i18n/spdKeys.ts`), `displayName.ts`'s armor branches | **AC_TRANSFER ported 2026-09-20** from `ClassArmor.java` tag `v3.3.8`: the equipped class armor's detail action opens the armor picker, destroys the old class-armor shell, keeps the selected ability and charge, and adopts the target armor's level, tier, glyph, curse-infusion marker, hardening and identification before spending one turn. The compact detail window has one action button, so Java's simultaneous `AC_DETACH` + `AC_TRANSFER` menu is simplified to detach first when a seal is present, then transfer after detaching; this preserves both operations without inventing a second action-menu layer. The crown choice and Rat King exchange convert the worn armor to the hero's per-class subclass id (`warriorarmor`/`magearmor`/`roguearmor`/`huntressarmor`/`duelistarmor`/`clericarmor`, Java's subclass names lowercased), keeping tier/level/glyph/curse/identify through the unchanged scalars and the seal for Warriors only (`ClassArmor.upgrade()` affixes it in the `WARRIOR` case alone). Names come from SPD's own `items.armor.<class>armor.name` keys (the Cleric's from the `port.items.armor.clericarmor` key with tag-`v3.3.8` translations, the checkout's messages predate it) and descriptions through the same `.desc` swap every armor uses; equip/detail/stylus/detach/blacksmith-harden/save-load all treat the six ids as armor, and bones excludes them (`ClassArmor.bones = false`). Still open: the class-armor sprite tier (`updateArmor()` art). |
...
| Rogue `DeathMark.activate()` + `DeathMarkTracker` (`DURATION = 5`, `setInitialHP`, `chargeUse`'s `0.707^DOUBLE_MARK`) and `Char.isAlive()`'s `HP > 0 \|\| deathMarked` | `activateDeathMark`, `tickDeathMark`, `processFearTheReaper`, the `kill()` gate, `simulation/warriorAbilities.ts`'s sibling `armorChargeUse` overrides | Ported (2026-09-16), the first of the Rogue's three. The mark is a five-turn per-creature payload (`deathMarkTurns`/`deathMarkInitialHp`, saved with the floor) rather than a real buff object, because this port's buff map stores durations only - so the mark has no icon and no `announced` line, which is the one stated reduction. Java spends **no time** on the cast (`hero.next()`), which the port reproduces exactly: the clock does not advance, the monsters do not act, and the hero simply acts again. A marked creature's HP is *not* allowed to end it - `kill()` refuses while the mark is up and runs `processFearTheReaper` instead (Java's `Char.damage()` branch), so the target stays in the fight at 0 HP for up to five of its own turns and then dies on the tick that clears the mark; `DEATHLY_DURABILITY` pays the hero `round(initialHP * 0.125 * points)` as a barrier at that moment, and `FEAR_THE_REAPER`'s ladder is real (rank 2+ terrifies the target too, rank 3+ reaches every hostile within path distance 3 via the port's own `distanceMap`, rank 4 terrifies those as well) except that `Buff.prolong(...).object = hero.id()`'s source id has no field on this port's terror buff. `DOUBLE_MARK` is Java's two-sided latch: armed costs `0.707^points` (30/50/65/75% off) and is consumed by the next mark, and with the talent ranked the cast re-arms it. Divergence (deliberate, 2026-09-17): an empty-cell cast reports SPD's generic armorability.no_target - Java asks Messages for a deathmark.no_target key that exists in no properties file (checked v2.1.4 and v3.3.8), rendering its missing-key text, which this port does not reproduce. Browser-verified live: the cast cost exactly 25 charge with the clock unmoved and input still live, a 999-damage hit left the target in the roster at 0 HP (Java's clamp, not a negative value) with terror+cripple on it and on a neighbour, the five ticks then killed it and granted exactly 20 barrier for a 40-HP mark at rank 4, the three-mark sequence cost 25 / 12.496 / 25 (and 7.498 with `HEROIC_ENERGY` 4), and ability, charge, tracker and both mark fields survived a save/load round trip. **28th matrix 2026-09-19 (`MONSTER_ANALYSIS_ROGUE_ABILITIES.md`): two real defects fixed, both pinned in `test:simulation` - re-marking now extends the window (`Buff.affect` spends additively) instead of resetting it to 5; the bankable `DoubleMark` discount is now a same-round latch (any clock advance drops it, old armed saves load disarmed, matching the 0.01-duration tracker); the detach/barrier/kill-gate/Reaper ladder verified unchanged. Residual: a marked-then-corrupted (allied) target still pays the barrier; Java pays it only for non-allies. |
| Rogue `ShadowClone.activate()` + `ShadowAlly` (tag `v3.3.8`) | `activateShadowClone`, `spawnShadowClone`, `syncShadowClone`, `shadowClone`, `simulation/rogueAbilities.ts`, the `takeAllyTurn` sync hook, `shadowclone` in `PORTED_ARMOR_ABILITIES` | Ported 2026-09-19 (20th matrix, `MONSTER_ANALYSIS_SHADOWCLONE.md`): without a clone the Rogue summons onto a random free `NEIGHBOURS8` cell (passable only - the clone does not fly, so no hawk-style `avoid` half) for 35 charge plus a turn, dispelling invisibility; with one out the ability aims and directs through the shared `directAlly` defend/follow/attack orders for 0 charge and no turn (`armorChargeUse`'s new `cloneSummoned` override, mirroring the hawk). The ally rides the generic ally combat/movement/orders branch with `allyKind 'shadowClone'` (persisted through save/load like every ally). Stats are Java's: `HP 80 + round(0.1 * PERFECT_COPY * (15 + 5*heroLevel))`, `accuracy heroLevel + 9` / `evasion heroLevel + 4` (the port's 10/5 base is Java's own attack-10/defense-5 scale), damage `10-20 + round(0.08 * SHADOW_BLADE * heroMean / attackDelay)`, armor `round(0.12 * CLONED_ARMOR * heroArmorMean)` - the pure arithmetic lives in `simulation/rogueAbilities.ts` and is pinned in the suite, re-read on every clone turn (hawk precedent) from the means of the hero's current ranges rather than live per-swing rolls. Stated reductions: the `Int(4) < points` weapon-enchantment and armor-glyph/proc shares need a gear-proc path for non-hero attackers that does not exist; the double-speed return (`speed x2` while WANDERING back to the hero) is not modeled; `PERFECT_COPY`'s interact range has no char-interaction system to read it; the sprite is the hero sheet darkened (`tint 0x555566`), not a real `ShadowSprite`. Type-check/simulation suite green; browser verification owed per ROADMAP.md section 10. |
| Duelist `Challenge.activate()` + duel (`DuelParticipant`, `SpectatorFreeze`, `EliminationMatchTracker`, tag `v3.3.8`) | `activateChallenge`, `tickDuelParticipant`, `detachDuel`, `closeTheGapRange`/`eliminationMatchFactor`/`invigoratingVictoryHeal` in `simulation/duelistAbilities.ts`, `heroShieldPoolTotal` + `duelTakenDmg` ledger, `spectatorFreeze`/`duelParticipant`/`eliminationMatch` buff rows, `challenge` in `PORTED_ARMOR_ABILITIES` | Ported 2026-09-19 (21st matrix, `MONSTER_ANALYSIS_CHALLENGE.md`): visible-enemy targeting with Java's five refusals (`no_target` through the shared `armorability` parent, as `Messages.get` resolves it), `CLOSE_THE_GAP` blink `1 + points` over the target-rooted path map (unrooted only, strictly-closer + true-distance tie-break), unreachable/far refusals with the rooted shake, 10-turn freeze of every other non-ally non-NPC char (the `delayChar` half folded into the post-tick action block; boss duels freeze nobody, and Java's `BOSS_MINION` half of that condition is dead code), paired 10-turn `duelParticipant` with aggro, per-turn pairing checks on both sides (gone/same-side/> 5 detaches), the duel ledger snapshotted at the `absorbHeroDamage` boundary (all 15 hero-HP sites, deferred ticks, overkill and fury-to-1 included), `INVIGORATING_VICTORY` payout on death/conversion capped at missing HP, `ELIMINATION_MATCH` 3-turn `0.84^points` discount (stacking over heroic) armed on the living hero's detach - including silent 10-turn expiry via the ledger marker - and `Char.damage()` negation for frozen spectators in `attack()` and at the mob DoT tick. Stated residuals: bomb/trap/blast seams do not negate on frozen spectators, the freeze buff-lock is unmodeled, no frozen darkening is rendered, and `rooted()` is the `roots` buff only. Type-check/simulation suite green (120 checks); browser verification owed per ROADMAP.md section 10. |
| Huntress `SpectralBlades.activate()` (the aimed line with `findChar`'s `2 * PROJECTING_BLADES` wall penetration, `FAN_OF_BLADES`' `30 * points`-degree cone with the `1 + points` target cap and its prune-the-furthest loop, `hero.attack(ch, dmgMulti, 0, accMulti)` at half damage for everything but the primary, `SPIRIT_BLADES`' tracker) | `activateSpectralBlades`, `spiritBladesArmed`, the new `damageMultiplier` on `rollDamage`/`resolveAttack`/`attack` | Ported (2026-09-16), the third ability class with real behaviour. `Char.attack`'s `dmgMulti` is now a real parameter of the damage roll rather than a post-hoc scaling, so a secondary blade's half damage is `round(roll * 0.5) - armor` the way Java computes it, not `round(roll - armor) * 0.5`; its only caller is this ability and a `verifyCombat` check pins the difference. `FAN_OF_BLADES` reuses this port's line-for-line `ConeAOE` translation, and the target cap is Java's own loop (drop whichever candidate is furthest from the primary by true distance). **Two stated reductions.** **Correction 2026-09-19 (27th matrix, `MONSTER_ANALYSIS_HUNTRESS_ABILITIES.md`):** reduction (1) misread Java twice over and is replaced. `Talent.onAttackProc` (Talent.java 896-901) runs the *bow's* `proc()` - the nature block - not the melee affix: a consumed tracker now calls `applyNaturesPowerOnHit` for real (pinned in `test:simulation`), and the separate `x1.1` damage line is gone outright (Java's rank-4 `+0.1` is an enchant *proc-chance* term in `Enchantment.genericProcChanceMultiplier`, and unreachable at that - `Int(10) < 12` always consumes the tracker before `wep.proc` runs - so it is recorded not-ported, not owed). Confirmed matching: the detach sits inside the roll's branch (failed rolls leave the tracker armed for the remaining blades) and the turn-end clear matches Java's buff-act expiry. (2) `PROJECTING_BLADES`' wall penetration is implemented to Java's own order (a creature on a cell is found *before* that cell's solidity is counted, and the search aborts once the allowance goes negative) and was live-verified for that ordering - a blade reached a target standing on a non-passable cell - but the case the talent exists for (a target the hero's FOV still sees while the straight ray clips a solid cell) did not occur anywhere on the verification floor, so that specific geometry is unverified rather than claimed. Browser-verified otherwise: the primary took exactly its weapon's roll, a `FAN_OF_BLADES` fan hit two cone targets for exactly half each, a fourth candidate further from the primary was pruned by the `1 + points` cap, a consumed tracker now sprouts the nature plant instead of adding damage (the old rank-4 `x1.1` line was removed as invention in the same pass), a target out of FOV refused with SPD's generic `armorability.no_target` (Divergence, deliberate 2026-09-17: Java asks for a `spectralblades.no_target` key that exists in no properties file at v2.1.4 or v3.3.8, rendering its missing-key text, which this port does not reproduce), and the charge went 100 -> 75.2 (25 spent, 0.2 of regen from the spent turn). |
| Mage `WarpBeacon.activate()` + `WarpBeaconTracker` (placement gates, the `window_tele`/`window_clear`/`window_cancel` window, the same-depth teleport with `TELEFRAG` and the occupant push, the cross-depth `LONGRANGE_WARP` gate and its charge multiplier, `REMOTE_BEACON`'s `4 * points` placement range) | `activateWarpBeacon`, `placeWarpBeacon`, `openWarpBeaconWindow`, `warpToBeacon`, `teleportHeroTo`, the saved `warpBeacon` | Ported (2026-09-16), the Mage's first ability. Its two halves deliberately differ in cost, exactly as Java wrote them: **placing** spends a turn and no charge, **recalling** spends charge and no time - both still require the base charge to be available, because `ClassArmor.execute()` checks that before the ability runs at all. Cross-depth warps are gated on `LONGRANGE_WARP` and cost `1.833 - 0.333 * points` times the base; the same-depth landing reuses the port's existing teleport-and-push shape (`ScrollOfTeleportation.appear`'s own rule, including `TELEFRAG`'s exchange - `IntRange(10, 15) * points` on whatever is standing there for `min(5 * points, HP + shielding - 1)` on the hero, which can never kill) and the cross-depth one reuses the Lloyd's Beacon arrival path. **Two stated reductions.** (1) Java's `Dungeon.interfloorTeleportAllowed()`/`LockedFloor` gate has no model here, so the port stands in the boss floor itself: a warp *out* of a live boss fight is refused, since a boss floor's only exit in this port is the boss's death and Java would refuse it too. (2) Java's `branch` (the mine side-branch) is this port's single `miningBranchActive` flag as 0/1, and a cross-branch warp therefore also goes through `enterLevel()` rather than being impossible. Browser-verified live: placement at the hero's cell with the charge untouched and a turn spent, the three-row window in French, the recall landing exactly on the beacon for 35 charge and no turn, `TELEFRAG` rank 2 dealing 28 to an occupant (in Java's 20-30 band) and exactly 10 to the hero at 20 HP while shoving it to a free neighbour, `REMOTE_BEACON` refusing distance 2 without it and distance 5 with rank 1, the `depths` refusal without `LONGRANGE_WARP`, a depth-2 -> depth-1 warp for exactly `35 * (1.833 - 0.333*3) = 29.19` charge landing on the beacon cell, and a cross-depth warp refused while standing on depth 5. **29th matrix 2026-09-19 (`MONSTER_ANALYSIS_MAGE_ABILITIES.md`): one real defect fixed - placement now dispels invisibility like both recall halves (pinned in `test:simulation`); everything else verified unchanged. Residuals: the recall push omits the LARGE/openSpace clause (no open-space concept exists to hang it on), and scene damage rolls use the port-wide `normalRange` primitive rather than triangular `NormalIntRange`. |
| Rogue `SmokeBomb.activate()` (the six-cell path-distance/FOV/occupancy gate, `Blindness.DURATION/2` on every adjacent hostile, `HASTY_RETREAT`, `BODY_REPLACEMENT`'s `NinjaLog`, `SHADOW_STEP`'s discount and free action) plus the `Blindness` buff it needs | `activateSmokeBomb`, `placeNinjaLog`, the new `blindness` buff id, the monster-perception gate, `ninjaLog`'s MWL monster row/sprite/frame entries | Ported (2026-09-16), the Rogue's second. `Blindness` is now a real buff, and its whole effect is one line of Java's `Level.updateFieldOfView`: `sighted = c.buff(Blindness.class) == null && ...`, i.e. a blinded creature's field of view is *empty* - so a blinded mob cannot see or hunt the hero, which is what makes the escape work. The port gates its monster-perception line on it and leaves the damage-driven `seesHero` sites alone, matching `Char.damage()`'s unconditional HUNTING. `NinjaLog` is a real creature row (spawned as an ally, immovable, immune to terror/amok/charm, with `HT = 20 * points` and `drRoll()`'s `NormalIntRange(points, 3*points)` set at spawn) that `takeAllyTurn` returns from immediately - it exists to be attacked, which this port's existing ally targeting already does. `SHADOW_STEP` is the talent that makes the ability worth using while already invisible: `0.84^points` off the charge and **no time spent at all** (`hero.next()`). Stated reductions: `HASTY_RETREAT`'s fractional `0.67f + points` turns are rounded to whole turns by this port's buff map, and Java's HUNTING->WANDERING flip is reached here by clearing the sight flag the hunt is keyed on rather than a separate state field. Browser-verified live: a hero warped three cells for 50 charge with a turn spent, the adjacent rat blinded for 5 turns and dropped out of sight while a non-adjacent one was untouched, `HASTY_RETREAT` rank 4 granting haste **and** invisibility (4 turns after the cast's own tick), `BODY_REPLACEMENT` rank 2 leaving a 40-HP `[2,6]`-armor decoy on the hero's old cell, `SHADOW_STEP` rank 4 costing exactly `50 * 0.84^4 = 24.89` with **no turn spent** and the invisibility preserved, and an impassable target refused with nothing spent. **Two real bugs were found and fixed on the way, neither introduced by this ability**: (1) the blanket `Invisibility.dispel()` in `activateArmorAbility` was stripping the invisibility `HASTY_RETREAT` grants and would have defeated `SHADOW_STEP` entirely - the dispel is now per-ability at Java's own point, and Death Mark and Smoke Bomb are exactly the two abilities whose Java sources do *not* dispel; (2) `spriteAnimations`' three `"fps": 0` clips (Java's `new Animation(0, true)` static frames) crash MWG's `Animation` constructor on spawn - `rotLasher`'s two were a latent crash-on-spawn for the Dwarf King's summon and are now 1fps (visually identical for a one-frame loop), and `ninjaLog`'s is hand-recorded the same way since `tools/extract-sprite-animations.py`'s own source path is stale after this repo split out of the SPD checkout (a separate recorded fix). **28th matrix 2026-09-19 (`MONSTER_ANALYSIS_ROGUE_ABILITIES.md`): two real defects fixed - a new decoy retires existing logs first (Java kills them; `kill()` is ally-safe), and the log gains its INORGANIC half (bleeding/poison buff row plus the `inorganic` actor set, so toxic gas reads it too). Gates, discount, blind, log stats, Hasty Retreat, and the free shadowstep cast verified unchanged. |
| Huntress `NaturesPower.activate()` + `naturesPowerTracker` (an eight-turn window, `Hero.speed()`'s `2 + 0.25*GROWING_POWER`, `SpiritBow.proc()`'s `NATURES_WRATH` plant and `WILD_MOMENTUM` extension) | `activateNaturesPower`, `naturesPowerSpeedFactor`, `applyNaturesPowerOnHit`, the saved tracker fields | Ported (2026-09-16). The hero's own speed is `Hero.speed()`'s line exactly, expressed as the turn-cost divisor this port's `getActionTurnCostMod` uses for every other speed effect (Haste, Swiftness, Flow), and the window counts down on the actor clock like every other buff. `NATURES_WRATH` sprouts Java's own five harmful plants (`NaturesPower.harmfulPlants`: blindweed, firebloom, icecap, sorrowmoss, stormvine) `Random.Int(12) < points` of the time, placed *under* whatever the hit struck and fired immediately through this port's existing mob-plant path (`triggerMobPlantAt`), so each plant's non-hero half is the same code a monster stepping on one already runs. `WILD_MOMENTUM` extends the window by its own rank on a killing hit, twice per cast (`extensionsLeft = 2`). **Two stated reductions.** (1) `SpiritBow.speedFactor`'s own `speed += (8 + points)/24` while the window is up - a faster *bow*, not a faster hero - is not modelled: this port's thrown attacks share one cost path with every other missile and it has no per-weapon speed factor to hang it on. (2) The hero's speed factor is applied through the action-cost divisor, which is exact for movement and ordinary actions but not the same `float` arithmetic Java does inside `speed()`. Browser-verified live: the cast costing 35 charge and one turn, `GROWING_POWER` rank 4 giving a speed factor of exactly 3 with a wait action advancing the clock by a third of a turn, `NATURES_WRATH` rank 4 triggering a plant on 128 of 400 direct calls (32% against Java's 33.3%) and - through a real thrown attack - landing `daze`/`cripple`/`poison` on the target, `WILD_MOMENTUM` rank 3 extending 5 -> 8 -> 11 turns across two kills and refusing a third (and never on a live target), and the ability, its charge and both tracker fields surviving a save/load round trip. **Reduction (1) closed 2026-09-18:** `SpiritBow.speedMultiplier()`'s `+= (8 + GROWING_POWER)/24` while the window is up is now a turn-cost divisor on bow shots only (a one-shot `pendingBowNpDivisor` consumed by the shared `spendTurn` port, so thrown/zapped specials keep the full cost; Java folds it additively into Furor's multiplier, this port divides the whole action cost - stated). Sniper specials are exempt in Java and do not exist here, so no gate. **27th matrix:** the nature block now also fires on consumed Spirit-Blades tracker rolls during blade attacks (Java's `Talent.onAttackProc` runs `bow.proc()` on any landed hero attack), via the same `applyNaturesPowerOnHit` with its own gates. |
| Huntress `SpiritHawk.activate()` + `HawkAlly` (tag `v3.3.8`) | `activateSpiritHawk`, `spiritHawk`, `takeSpiritHawkTurn`, `applyGoForTheEyes`, `spiritHawkSpeed`/`spiritHawkViewDistance`/`spiritHawkDodges`/`goForTheEyesEffect` in `simulation/huntressAbilities.ts`, the `spiritHawk` monster row, the `blobImmune` set | **Ported 2026-09-19 (27th matrix, `MONSTER_ANALYSIS_HUNTRESS_ABILITIES.md`).** 35 charge summons onto a free passable-or-avoid neighbour (`no_space` spends nothing), redirect through the shared direct orders is free (`chargeUse` 0 with a hawk out), the summon dispels invisibility and spends a turn. The ally is Java's: HP 10, 60/60 attack/defense, `NormalIntRange(5, 10)` damage, no armor, flying, blob-immune, no XP, never auto-hunts (orders only), `2 + SWIFT_SPIRIT/2` speed with the 100-unit lifespan spent by actor time, outright dodges for `2 * SWIFT_SPIRIT` attacks, the `GO_FOR_THE_EYES` table (blind 2 / blind 5 / blind 5 + cripple 2 / blind 5 + cripple 5), FOV shared with the hero at hawk sight, lifespan and dodges persisted with the run. **Divergence (deliberate):** sight stays clamped to 6-8 (the constructor's `GameMath.gate`); Java's `act()` recomputes it ungated (up to 10 at rank 4). **Residuals:** hawk-expiry `hero.interrupt()` is not modeled; the flat-1 turn cost follows the port-wide ability convention (Java spends `attackDelay()` on blades, `TICK` on the other two). |
| `Ratmogrify` (`baseChargeUse = 50`, `talents()` = `RATSISTANCE`/`RATLOMACY`/`RATFORCEMENTS` + `HEROIC_ENERGY`) | `grantRatmogrify`, `ratmogrifyChargeUse`, `useRatmogrify` | Partially ported. The charge cost is now real (50, scaled by `HEROIC_ENERGY`) and **a real turn-cost bug was fixed in the same pass**: `useRatmogrify` both spent its own turn and returned `true` to the action adapter that spends one, so every Ratmogrify cost two turns. The six-turn transformation itself keeps its documented simplification (no `TransmogRat` actor, so the sprite and kind stay), which is exactly why the three rat talents used to grant no tier-4 pool here. **2026-09-19:** Ratmogrify now has its own `class: "any"` row in `talent-rules.mwl` (granted by the Rat King to any class, never offered), so the tier-4 tab opens with all three rat talents plus `HEROIC_ENERGY`, and `RATLOMACY` (permanent ally on transformed rats) and `RATFORCEMENTS` (ally-rat spawn on empty self-cast) run off their ranks. Still open at that point: `RATSISTANCE`'s damage factor (needs the missing `TransmogRat` actor's damage path), and the same pass fixed the message key - Java derives it from the ability class's package, so unlike the eighteen class abilities Ratmogrify's key has no class segment (`actors.hero.abilities.ratmogrify`, `actors.properties:609`). **2026-09-19 (later same day):** `RATSISTANCE` is ported too - `simulation/ratmogrify.ts` carries the pure `0.9^points` factor and `resolveHeroAbilityAttack` folds it into the attack multiplier for a `ratmogrifiedTurns` attacker that is not an ally (Java's `!allied` gate), reading the hero's own rank. Stated rounding difference: Java truncates the transformed roll to int before the rest of its chain, while this port's float pipeline rounds once at the end, so a `.5` boundary can differ by 1 - the same simplification every other multiplier here already carries. All three rat talents now run; the `TransmogRat` actor itself (sprite, separate damage path) remains the documented simplification. |
...
bleeding 0, the 9999 state markers) or a real application site's value (paralysis
...
**Trace-bisected 2026-09-19; two corrections to the parity narrowing above.** First, the
'resync perfectly' reading for seed42/depth8 does not hold on current traces: 0/60 post-gap
lines match and the totals are 4390 (Java) vs 6582 (port) - a retry cascade, not an isolated
three-draw gap. The port's secret-branch guard was re-instrumented and behaves correctly there
(honeypot guardRounds 1,1,2), room sizes spot-check identical while every position differs, and
both sides converge attempts=1, so the split is a placement-outcome divergence from identical RNG
inputs - a deterministic geometry/logic difference on the placement path (branch order
unexcluded), not an RNG-call difference and not secret selection. Second, seed999999999999/depth9's
split is NOT in maze-growing: room labels, all painting, and all three trap class rolls are
identical through draw 22625, and the first difference is one extra Java float between trap
chances#1 and element#1 in `paintTraps` (net java+3 overall; 3 extra TS ints sit at the tail,
pointing at item-substream ordering) - maze-growing's draws match, so the 'shared cause via a maze
connection to a secret' inference is withdrawn for this floor. `PortedFloor.attempts` is now surfaced
(and printed by `--write-ts`) so the next pass reads the port attempt count the way the Java dump
header already reports it. The checkout-provenance warning stands and hardens: HEAD here is custom
history with `v3.3.8` not an ancestor, so the next pass must verify candidate Java behavior against
the checkout worktree that produced the dump, not only the `v3.3.8` tag.

**Resolved 2026-09-19 - both open floors are one oracle version-skew, not two port bugs.**
The scratch Java oracle's `core/` is v2.1.4-era (merge-base with `v3.3.8` is `v2.1.4`;
newest upstream commits on its levelgen files are v2.0.0-v2.1.4), predating upstream's
v2.2.0 "redesigned crystal path rooms". This port implements the post-redesign
`CrystalPathRoom` from its cited references (`v3.3.8` and `4.0.0-beta`, both verified by
`git show` to carry the center-strip `canConnect`), so every floor containing a crystal
room diverges from the stale oracle by design. User decision same day: keep the
`v3.3.8`/`4.0.0-beta` behavior and classify both blocks as oracle skew - no port change.

*seed42/depth8 (draw 321, build phase).* Port-side stack attribution puts draws
321-323 inside `LoopBuilder.randomBranchAngle` (a 5-float call, draws 321-325); Java
draws three `bits=31` ints at the same indices, then both streams reconverge
value-for-value at 324-335 (`next(bits)` advances the LCG state identically whatever
`bits` is, so equal draw counts resync). The three Java ints are the end of a
`createBranches` iteration with one connecting room (`Int(3)` per connecting room +
`Int(3)` once) plus the next iteration's `Random.element(branchable)`; the port never
reaches them because its `placeRoom` for the same inputs fails first and retries
(port 6582 draws vs Java 4390 - the retry cascade). The failing call, logged via a
temporary probe (since reverted): prev connection room (15,17,20,22), branch angle
253.5306396484375, next special room settling at (7,20,15,27), `connect()` false. Angle
selection is exonerated (the winning candidate leads by ~113 degrees - no float-ulp
flip can move it), and `placeRoom`'s float ops were checked line-by-line against
`Builder.java` with no divergence. The flip is `CrystalPathRoom.canConnect(Point)`: the
shared-edge candidates (15,21),(15,22) pass the worktree's refuse-center rule
(center ~(11,26): x=15 is not 11, height even so no `center()` roll or refusal - Java
connects) and fail the port's center-strip rule (`|15-11|=4`, `|21-23.5|=2.5`,
`|22-23.5|=1.5`, none `< 1` - port refuses). This also retires the 2026-09-19 secret-
selection dead end above: secret *content* matches (honeypot secret on both sides,
guard instrumented and correct) - the three-int gap was never secret selection.

*seed999999999999/depth9 (draw 22626, paint phase).* A new scratch facility - an
env-armed stack window in the harness's `TracingRandom`
(`LEVELGEN_STACKWIN=seed:depth:lo-hi` with `LEVELGEN_TRACE=true`, writing
`levelgen_stacks_<seed>_<depth>.txt` next to the traces) - gives Java-side call sites:
draw 22625 is Java's last `paintGrass:406` float, and 22626/22628/22630 are its trap
`chances` with 22627/22629/22631 its trap `element`s. So `paintTraps` runs an identical
`[chances, element]x3` on both sides; Java's `paintGrass` simply draws exactly one more
float (one more EMPTY-among-grass-patch cell), shifting the trap phase one index with a
count-resync at 22631-22635 (equal counts keep LCG states synced, so later equal-index
values coincide while meaning different ops - including the phase-shifted
`spawnWandmaker` draws). Withdrawn: the maze-`growMaze` theory (already withdrawn), the
`paintTraps`-op theory, and the "3 extra TS ints at the tail are item-substream
ordering" reading. The one-cell pre-grass map difference is the same crystal skew made
visible one stage later: the crystal entry door lands at (40,22) on the port vs (41,22)
on Java (center-strip admits only x~=40 of the shared walkway edge, refuse-center
admits 40 and 41 - the same `element(doorSpots)` draw then picks different cells with
an identical draw count, invisible in traces), and an internal crystal door lands at
(40,24) vs (38,26), with the shifted trap cells (port (14,4),(40,16),(46,17) vs Java
(19,4),(18,8),(46,15)) and wandmaker/item shifts making up the 47-cell total. Build,
room rects, water, trap tables, and trap chances logic are identical throughout.

Net for section 9: 26/28 floors `TRACE-IDENTICAL`, 2/28 classified oracle version-skew
with the mechanism proven per draw. No maintained test is owed: no shipped behavior
changed, and `parity:levelgen` keeps reporting both blocks' first-divergence indices,
so any *new* divergence is still caught.
...
(no discrete-meter UI need). Correction 2026-09-19 to the removal claim first written
 here (`InventoryItem.sourceClass` gone in 0.15.0): it never happened - the
 published 0.14.0 artifact already has `category`, no `sourceClass`, identical to 0.15.0
 (checked tarball to tarball). The stashweapon writer was new code that never compiled
 under either version; it rides a cast because `sourceClass` is port-owned data. No
 changelog note is owed; the "Closed" paragraph below still opens on the removal premise
 and needs its first clause reworded.
**Closed 2026-09-19 (ROADMAP 11A's acceptance gate, port side):** adopted-surface
regressions are exactly what `tools/verifyMwgCompatibility.mjs` now guards against - 15 adopted-surface
declarations (Game/Window/WindowStack + `handleAction`, TabbedList/IconGrid, AnimatedSprite/
Tweener, ParticleEmitter/SpriteSheet + `rect`, ListView `tapRow`, Camera `shakeScreen`,
ReactionTable, Scheduler `restore`, SimulationRuntime/EventPresentation/PresentationQueue/
`advanceToInput`, TargetingController/MultiTurnBeam, EntityRegistry + `add`, contentCatalog/
validateCatalog/`craft`) plus a negative `tableReferences` probe (a bogus reference must
still report `MWL_TABLE_REFERENCE`), all renderer-free and offline in `test:mwg` (19 checks).
`setAssetMap`, `coneSector` and `ScreenEffects.sequence` are deliberately absent from that
list - shipped but not adopted, each with its reason in this section - so their absence is
the assertion. The framework-side half of the gate (renderer-free tests, minimal example,
save notes, API report entry in MWG before any new proposal) is satisfied by process: the
only open upstream items (P1 caller-chosen ids, P2 Types2D values, the ScreenEffects
gradient-hook candidate) are recorded as open and unadopted, so no adoption checkbox here
was added without a released, declaration-checked version (`npm run mwg:check` confirms
pin/installed/latest all 0.15.0).
...
| `ui/BuffIndicator.java` + `ui/BuffIcon.java` - 7x7 (small) or 16x16 (large) icons from `buffs.png`/`large_buffs.png`, indexed by `BuffIndicator`'s own constants, skipping `icon() == NONE` | `statusPane.ts`'s `BUFF_ICON` map, all 12 buffs this port models | Ported (real indices, incl. Monk `Focus`'s `MIND_VISION` icon and its green hardlight). **Correction, 2026-09-19: the large sheet is now wired in too** (see `SPDSettings.interfaceSize()`'s row below - `ui_large_buffs.png` sat on disk unused since the asset-extraction pass; `StatusPane`'s large branch now swaps to it, 16px cells/16-wide `TextureFilm`, on the same toggle that drives the 1.5x pane scale). Simplified: the row is rebuilt outright rather than tweened in and out with Java's `AlphaTweener`. **Extended 2026-09-19: `BuffButton`'s overlays are now live** - the large-mode `iconTextDisplay()` countdown bottom-right (FlavourBuff `turns+1` per `visualcooldown() == cooldown()+1`, Burning/Ooze/Poison plain `(int)left`, POSITIVE/NEGATIVE tint, 0.7 alpha, halved when wider than the icon) plus the `iconFadePercent()` grey wash (`0x666666` at 0.8 alpha, Java's `0xCC666666`, from the top with the same ceil-below-half/floor-above pixel snap) on every icon without text, small or large. Fury/cloak/focus/berserk/hunger show neither, each for its Java reason (no overrides; Shadows' fade is 0 and its value is a sentinel here; Berserk's readout and MonkEnergy's count have no modelled quantity). Poison's text is a stated divergence: Java shows the damage pool, this port its remaining turns. Pinned headlessly (`tools/verifyBuffOverlays.mjs`, 6 checks); the scene plumbs `hero.buffs` durations into the pane |
| `windows/WndInfoBuff.java` - clicking a `BuffIndicator` icon opens the buff's real name/description, `{0}` filled from `Buff.desc()`'s own turns-remaining value | `src/ui/buffInfo.ts` (the id-to-message-key table and the `{0}` substitution), `src/ui/buffInfoWindow.ts` (the popup), `statusPane.ts`'s `onBuffClick` | **Ported 2026-09-18.** Every hero-visible icon in `BUFF_ICON` resolves to its real SPD name/desc pair and shows the actual `hero.buffs[id]` turns value where Java's own text has one (`cloak`/`shadows` and `fury` do not - Java ends both by condition, not a clock, so no `{0}` is substituted for either); `Hunger`'s two icon states go through their own real key shape (`hunger.hungry`/`.starving` name, `desc_intro_<state>` prefixed onto the shared `hunger.desc` body) since it is a real buff too, just not one the generic id table can drive. Browser-verified live (French locale): clicking the burning icon opened "En feu" with "Tours de feu restants : 7" from a live `hero.buffs.burning = 7`. |
...
| `StatusPane` hero avatar and compass placement | `src/ui/statusPane.ts`, `main.ts` | Ported for starting cloth armor: HeroSprite avatar crop, level at (27.5,28), XP at y=0, compass centered on portrait. **Portrait rule now exact (2026-09-18, checked against `HeroSprite.avatar()`/`Hero.tier()` at tag `v3.3.8`)**: the class sheet's own `(1, tier*15, 12, 15)` cell under Java's exact 0..6 clamp (0 no armor, 6 `ClassArmor`). Both ends are unreachable here (armor is never unequipped to nothing; class armor is not a ported item), and the `HeroDisguise` class-swap has no source (its only producer is the unported `CursedWand` random effect) - the old "avatar does not change with armor" simplification is withdrawn. Port-only stats appear on hover rather than in WndHero. **`WndInfoBuff`'s click-to-info is ported (2026-09-18, its own row above); the rest of `WndHero` (its tabbed Stats/Talents/Buffs shell) is not** - this port's equivalents are three separate entry points (the avatar's flat stat popup, the toolbar's own talent-spend window, and now each buff icon's own popup) rather than one tabbed window, which is a UI-shape difference, not a missing behaviour: every stat WndHero's Stats tab shows is already surfaced, and the Talents tab's browse-and-spend function already exists. **Talent-point-available blinking is ported too (2026-09-18), as a divergence (deliberate)**: `StatusPane.talentBlink`'s real trigger (any unlocked tier still has an unspent point) drives a static yellow corner dot (`talentDot`) rather than Java's animated avatar-tint cosine pulse - this port has no avatar-tint layer for any state yet, including Java's own low-HP pulse, so a static dot is the honest stand-in rather than invented tint art. It also tracks the live unspent-points state directly instead of Java's expiring 10s timer, so it cannot go dark on a still-unspent point - a small stated improvement, not a bug. Browser-verified live via a pixel diff of the dot-off/dot-on states. BusyIndicator and CircleArc (the animated tint layer itself) remain unported. |

| `CharSprite.flash()` (tag `v3.3.8`) - the brief full-white hit pulse `Char.attack()` fires alongside the damage number/blood-burst pair on every landed hit | `showDamage`, `allyIdentityColorAdd`, the per-frame `colorAdd` fade-out loop | **Ported, 2026-09-19, plus a real bug fix found in the process.** This port's `AnimatedSprite`s already expose a real renderer seam for exactly this (`colorAdd`, an additive tint channel, with a per-frame fade-out loop whose own comment already explained the design), but nothing had ever set it - the trigger was missing entirely, so no creature had ever visibly flashed on taking damage despite the fade-out machinery existing. Wired into `showDamage`, the one shared choke point every real damage-application call site already routes through. **Bug found and fixed in the same pass**: `colorAdd` is also an ally's persistent identity tint (`allyIdentityColorAdd` - Sheep 0xdddddd, EarthGuardian 0x997744, Lotus 0x55aa66, the shared 0x5577aa default for mirror/ghost/ninjaLog/spiritHawk/afterImage/shadowClone), and the fade-out loop unconditionally zeroed `colorAdd` every frame regardless of that baseline - so an ally's tint was being wiped to 0 within one frame of spawning, independent of this pass's own flash trigger (confirmed live: a spawned Sheep's `colorAdd` read back as 0 within 300ms even before the flash existed). The fade-out now restores each creature's real baseline (`allyIdentityColorAdd(isAlly, allyKind)`, 0 for a non-ally) instead of hard-zeroing, extracted into a shared function so the spawn-time assignment and the fade-out read the identical rule. Live-verified both halves: a damaged hero flashes to `0xffffff` then fades to `0` within one frame; a damaged Sheep flashes then fades back to its own `0xdddddd`, not to 0. Java's real 50ms timer collapses to this port's one-frame flash at its own tick rate - close enough without a dedicated timer, the same class of simplification `banner.ts`'s tint fades already use. See `ROADMAP.md`'s sprite/effect-animations line. |
| One-shot monster death/zap particle bursts (tag `v3.3.8`): `DM300Sprite.onComplete(die)` (`BlastParticle` x100 + BLAST), `PylonSprite.play(die)` (x20 + BLAST), `GuardSprite.play(die)` (`ShadowParticle.UP` x4), `SuccubusSprite.die()` (`Speck.HEART` x6 + shadow x8), `GhostSprite.die()` (`ShaftParticle` x4 + `Speck.LIGHT` x3 - both the quest NPC and the Dried Rose summon share `GhostSprite`), `WardSprite.zap()` (ward-blue x2 + RAY) and `WardSprite.die()` (ward-blue x10) | `deathBurstsFor`/`wardZapBursts` in `src/simulation/deathBursts.ts` (the file-size refactor's forty-seventh extraction, 2026-09-20 - pure spec table, pinned behaviorally by `tools/verifyParticles.mjs`), `playDeathBursts` in the scene, the shared kill path, `takeWardTurn` | **Ported, 2026-09-20.** The 17-file Java mob-sprite emitter audit behind this is inventoried in the module header: the six one-shot sites above are live with Java's own counts, colors (`BlastParticle` 0xEE7722, `ShadowParticle` 0x440044, `Speck.LIGHT` 0xFFDDDD00, `WardParticle` 0x88CCFF), lifespans, speeds and samples (`blast.mp3`/`ray.mp3` are real SPD assets). Two stated reductions: particles are plain squares (HEART's pink heart frame and ShaftParticle's streak become tinted squares - counts/colors/motion transfer, textured shapes do not), and the burst is gated on hero FOV (Java always emits but its camera culls off-screen sprites, so this is the same visible result including the sound). Still open, recorded in the module header rather than assumed closed: the twelve continuous `pour` auras (DM300 supercharge sparks, the four elemental auras, Eye charge, FetidRat stench, the four fist auras, Golem teleport, Goo spray + pump-up cells, Lotus leaves, Necromancer summoning bones + the Spectral twin's shadow summoning, PhantomPiranha sparkles, RotHeart cloud) need a persistent-emitter layer, plus `WardSprite`'s DeathRay beam, attacker flash and 2s death fade. See `ROADMAP.md`'s sprite/effect-animations line. |
| `SPDSettings.interfaceSize()`'s `large` variants throughout (`StatusPane`'s 128x9 bars, `large_buffs.png`, `InventoryPane`'s wide layout, `GameLog`'s 5-line mode) | `dungeonScene.ts`'s `interfaceSize` field + `toggleInterfaceSize` action, `statusPane.ts`, `gameLog.ts` | **Correction, 2026-09-19: this row was stale - the toggle itself, `StatusPane`'s 1.5x branch, and now its real `large_buffs.png` icon swap and `GameLog`'s 3/5-line switch are all ported.** `StatusPane` renders its bars/avatar/buffs at a uniform 1.5x scale rather than Java's own distinct large pixel cuts for each element (stated simplification, `statusPane.ts`'s header comment), but the buff icons specifically now come from the real separate `BUFFS_LARGE` sheet, not a stretched copy of the small one. **Closed 2026-09-19, both halves:** `BuffButton`'s overlays (own row above), and now `InventoryPane`'s wide layout - large interface mode switches the bag grid to Java's own 10-column arrangement (5 equipped leading, the 20-item page after, three full rows in a 302x168 window; `src/ui/bagLayout.ts`, footer/right-edge chrome re-anchored, narrow geometry provably unchanged). Pinned headlessly (`tools/verifyBagLayout.mjs`, 3 checks). The window itself stays this port's tabbed popup rather than Java's `uiSize == 2` bottom pane - the standing bag-UI simplification, not a gap in this row |
| `SPDSettings.zoom()` + `SPDAction.ZOOM_IN`/`ZOOM_OUT` (`PLUS`/`EQUALS`/`MINUS`, `GameScene.create()`'s `gate(minZoom, defaultZoom + zoom(), maxZoom)`, tag `v3.3.8`) | `src/settings.ts` (persisted integer offset under Java's own `zoom` key), `src/scenes/dungeonScene.ts` (camera zoom + key handling), `src/ui/portWindows.ts` (the settings row), `main.ts` (the bindings) | Ported - the offset (default 0) drives the dungeon camera as `3 + offset`, adjustable with Java's own `+`/`-` keys (numpad twins included) and a port-original `- level +` row under the settings window's real `DisplayTab` title; a mid-run change re-zooms the live camera through a subscriber, no scene rebuild. Pinned headlessly by `tools/verifyDisplaySettings.mjs` (offset gating, base mapping, subscriber fan-out). **Stated simplifications**: the gate is a fixed `[-2, +3]` (Java's is screen-derived around a density-derived default), Java has no settings row for zoom (that chrome is new, the preference underneath is ported), and pinch-to-zoom (`CellSelector`) stays unported |
| The bag, itemised on screen | `InventoryWindow`, item-action windows, `itemDisplayName`, artifact definitions in `artifacts.mwl` | Simplified - it separates equipment from carried items, exposes affixes/curses, supports concrete weapon/armor/ring instances and activates food, potions, scrolls, rings and armor. MWL-authored artifact descriptions are now shown in the inspect detail. **Correction, 2026-09-14: "descriptions for the remaining item families remain unported" was stale.** `inventoryPanel.ts`'s `entry()` already resolves a real description for every family through `MWL_CONSUMABLE_DESCRIPTION_KEYS` (potions/scrolls/seeds/stones/food/bombs), `MWL_MISSILE_DESCRIPTION_KEYS`, `MWL_EQUIPMENT_DESCRIPTION_KEYS` (weapons/armor/wands/rings), and the artifact table, each backed by a real authored MWL row keying a real Java `.desc` message - a fallback chain checked in that order, with `context.itemDescription` free to override any of them first for dynamic per-instance text. Sub-bags remain the one genuinely unported half of this row. **Bug fix, 2026-09-15: the dedicated artifact equip-slot icon silently recognized only three ids.** `inventoryPanel.ts`'s slot lookup (`rows.find(...)`) and `inventoryWindow.ts`'s `category()` "equipment" tab test each hand-listed `cloak`/`hourglass`/`holyTome` (or `holytome`) only - a real, hand-maintained duplicate of the artifact id set that fell out of sync the moment `chalice`/`cape`/`toolkit` were wired into `generatedInventoryItem`'s generation switch: those three (and, this session, `beacon`) rendered correctly in the carried list but never took the dedicated slot icon or counted as "equipment" for the Gear tab (confirmed live in a browser before the fix - the slot showed empty, or an unrelated carried item, while a Chalice sat only in the general list). Fixed by deriving both checks from `getAllArtifactIds()` (`src/items/artifacts.ts`, itself sourced from `artifacts.mwl`) instead of a third hand-written list, so a future artifact only needs its one line in the generation switch to also get the slot/tab right. Checked the casing complaint this raised too: `inventoryWindow.ts`'s check compares an already-`.toLowerCase()`'d id against `'holytome'`, and `inventoryPanel.ts`'s compares the raw id against `'holyTome'` - each is correctly cased for its own comparison context, so the visual inconsistency between the two files was not itself a bug. **Bug fix, 2026-09-19: the equipped-weapon slot's name never updated after equipping a real found weapon.** `refreshInventoryPanel` built the slot's `weaponName` from `t(CLASSES[this.heroClass].weaponKey)` unconditionally - the hero's *class's* starting-weapon key, not `this.weaponSourceClass` (the field `equipWeapon` actually sets to the real Java class, e.g. `'longsword'`, and the field every real gameplay site - weapon-ability lookup, flail-check - already reads correctly). A hero already wielding a found longsword still saw "worn shortsword" in the bag slot forever. Fixed by resolving `WEAPON_NAME_BY_CLASS[weaponSourceClass.toLowerCase()]` when `weaponSourceClass` is set and isn't the `'startingWeapon'` sentinel, falling back to the class key otherwise - live-verified both states (`"une épée courte usée"` unequipped, `"épée longue"` after `equipWeapon('longsword', ...)`, reverting cleanly). `weaponFrame` is untouched by this fix and stays the fixed per-class icon: this port has no per-weapon-class sprite frame data at all (`weaponReward`'s own bag-item frame is uniformly 96 regardless of `sourceClass` too, see `item-rules.mwl`), so that half is a real, separate, already-stated simplification, not part of this bug. **Added 2026-09-19: the equipped weapon's inspect detail had no description at all** - `MeleeWeapon.info()` (Java, tag `v3.3.8`) appends the weapon's own flavour text plus, for a Duelist only (`!(this instanceof MagesStaff)`, an exclusion that never applies here since this port has no `MagesStaff` item), its real `ability_desc` message key. `refreshInventoryPanel` now resolves `weaponSourceClass` to its `WEAPON_NAME_BY_CLASS` key, derives the sibling `.desc`/`.ability_desc` keys, and joins whichever exist (checked via `has()`) into `InventoryPanelContext.weaponDescription`, which `inventoryPanel.ts`'s `entry()` construction for the equipped-weapon slot now passes through as `description`. Live-verified via `window.__MWG__.currentScene`: forcing `heroClass = 'duelist'`, `weaponSourceClass = 'wornshortsword'`, then calling `inventoryPanel['showItem'](weaponEntry)` produced the real French flavour text with the Duelist `ability_desc` appended ("...un bonus de 27% de dégâts et touche à coup sûr...") - the correct, uncorrupted percentage confirming the `tools/i18n-extract.mjs` placeholder fix above alongside this feature. |
...
| The Waterskin/dew-drop mechanic (`MAX_VOLUME = 20`, `collectDew`, `consumeDew`'s 5%-of-max-HP-per-drop heal) | `collectDewdrop` in `items/consumables.ts` (moved 2026-09-20 as the file-size refactor's thirtieth extraction, behavior-identical - the scene keeps the one-line adapter the ground-pickup context calls), `groundPickup.ts`, `quaffPotion`'s waterskin branch, `WATERSKIN_MAX` | Ported with UI simplification: collection and the 20-cap are real; a full, fully-healed Waterskin now refuses ordinary dewdrop pickup and leaves the heap in place, while entrance/exit cells force the one-drop consume path like Java; drinking computes Java's minimum drops-needed formula, preserves unused water, and applies the Warden Shielding Dew overflow cap. It remains exposed through the port's quaff action rather than a dedicated quickslot/window; Vial of Blood is unmodeled. |
| Traps (`ToxicTrap`, `BurningTrap`, `PoisonDartTrap`, `GrimTrap`, `ExplosiveTrap`, `ConfusionTrap`, `CorrosionTrap`, `ShockingTrap`, `StormTrap`) | `placeHiddenTraps`, `triggerTrapAt`, `applyTrapBlast`, live fire/gas/electricity blobs, `TRAP_KINDS` via `mwg/roguelike` `Secrets` | Ported for gameplay consequences - real damage numbers (depth-scaled where Java scales); explosive traps affect every nearby character with Java's 0.67 off-center multiplier, and toxic/burning/confusion/corrosion traps seed spreading gas/fire blobs. **Shocking/Storm traps ported 2026-09-17**: no direct damage (Java deals none either) - they seed Java's `Electricity` volumes (`10` over NEIGHBOURS9, `20` over the distance-2 flood) into a persisted `electricity` blob whose `evolve()` paralyses by cell charge and zaps `round(Random.Float(2 + depth/5))` on odd charges, with Java's own `ondeath` line on a hero kill (landed in the default `foe` death-badge bucket, which predates the blob). Stated skips: the LIGHTNING sound only. **Conduction ported (2026-09-17)**: `evolveElectricity` (`simulation/javaBlob.ts`, pinned in `verifySimulation.mjs`) reproduces `spreadFromCell` - full power into every 4-neighbour-connected water cell, then minus one per charged cell instead of the generic diffusion - wired through the blob `advance` branch against the real `WATER` terrain. The mob-marking half is live too (`markHazardArea`/`markHazardMob` on both trap branches, both trigger sides). **Not ported**: Java's heap wand-charging (`gainCharge(0.333f)` on ground wands/staffs) - ground wands here draw from the hero's shared charge pool and carry no per-heap fractional state to bank into. Exact gas/fire volume cadence, destructible terrain, and non-explosive trap projectile/area presentation remain simplified **35th matrix, 2026-09-19 (`MONSTER_ANALYSIS_TRAPS.md`):** all 9 modeled kinds re-audited - Grim is `round(HT/2 + HP/2)`, not half-current plus quarter-max (fixed both branches; the mob branch also lost its invented armor subtraction, and the hero keeps Java's 90% cap); Explosive is a verbatim stock `Bomb.explode()` (`4+d..12+3d`, no falloff, no fire seed - fixed all three blast sites, which also kills the "0.67 off-center multiplier" sentence above: Java rolls full damage per char); Burning deals no direct damage at all (Fire 2 on NEIGHBOURS9, ignition via the fire tick - the `Int(2,5) x Elements` hit, instant burn and single-cell fire 4 are gone, and the branch logs nothing of its own now). `grimTrapDamage`/`explosiveTrapBounds` are pure in `src/simulation/combat.ts`, pinned in `test:simulation`. Verified: gas volumes/strengths, dart 4-8 minus armor with the exact poison set, shock/storm Electricity volumes and mob-marking, AntiMagic `drRoll()` semantics. Residuals: 25 unmodeled trap types (Alarm/Blazing/Chilling/Cursing/Disarming/Disintegration/Distortion/Flashing/Flock/Frost/Gateway/Geyser/Gripping/Guardian/Ooze/Pitfall/Rockfall/Summoning/Teleportation/TenguDart/Warping/Weakening/WornDart/GnollRockfall); no flamable-terrain destruction or heap/bomb chaining in trap blasts; passable (not `!solid`) seeding; Tengu depth-10 score; port's own 9-kind generation rotation. |
...
| `Hunger.act()` (`actors/buffs/Hunger.java`): `HUNGRY = 300`, `STARVING = 450`, `level += 1f/hungerDelay` (effectively +1, not the +10 this row's own text used to claim) per turn, crossing into STARVING deals an immediate `hero.damage(1,this)`, then `level` freezes and `partialDamage += STEP*HT/1000` every turn, applying `(int)partialDamage` (with carried fractional remainder) whenever it exceeds 1 | `simulation/hunger.ts`'s `advanceHunger` (`hungerStep`/`hungerPartialDamage` on `main.ts`), `eatFood`, `rings.mwl`'s `spdAdventureClock.hunger` | **Ported exactly** for the starvation-damage curve described below, but this row's own `STEP = 10` claim was itself a real, player-reported bug (2026-09-19), not a correct citation: Java's real per-turn climb is `1f/hungerDelay` (1, since `hungerDelay` defaults to `1f` and is only ever multiplied, never divided below 1) - not 10. `HUNGRY`/`STARVING` are Java's real, unscaled 300/450, so accruing at +10/turn made a hero hungry in 30 turns and starving in 45, ten times faster than real Java's 300/450. Traced to `rings.mwl`'s `spdAdventureClock` turn-clock authoring `hunger: 10` (with matching stale `?? 10` fallbacks in `dungeonScene.ts`, `sceneSimulation.ts` and `gameSimulation.ts`, and a `STEP = 10` constant in `simulation/hunger.ts` itself) - all now `1`. The starvation-phase formula this row otherwise describes was already correct and remains so: the port previously modeled starvation as a flat `max(1, round(maxHp/100))` damage tick once every 10 starving turns - a guess, not a translation of the real formula, and roughly half Java's real rate at low max HP (e.g. HT=20 takes 1 damage about every 5 turns in Java, not every 10). `advanceHunger` reproduces `partialDamage`'s real fractional accrual and carry-over, the immediate 1-damage-on-crossing, and the fact that `level` itself stops climbing once starving (Java's `isStarving()` branch never reassigns `level`) - previously this port kept incrementing hunger every starving turn via a separate tick counter, which the real Java never does. The `onhungry`/`onstarving` log lines fire from the same `newLevel` crossing check Java uses rather than persisted "warned" flags (removed from `HungerState`/saves - crossing is naturally self-gating). Simplified: no attack-delay/accuracy penalty while hungry (Java has none beyond the log line either), no exhaustion beyond HP damage. **Suite sync 2026-09-19:** the STEP fix above landed with stale STEP=10 expectations still in the suite (`test:simulation` aborted at hunger, hiding every check after it) - the warning/starving/crossing/adapter/hero-turn/hero-actions expectations now use STEP 1, the buff fixture is re-recorded for the poison draw removal (one scenario's burning roll reschedules, damage 4 to 2 with identical buffs/timing), and the full suite passes (118 simulation + 7 banner + 36 vault + 6 overlay). **Still not modeled: Java's no-rise gates** - hunger keeps accruing here while `WellFed`, on locked floors, during the intro, and in the arena/vault hiatus levels, where Java's `act()` skips the climb entirely; likewise the `Shadows` x1.5 slowdown (and the SaltCube divisor, whose trinket is unported) has no expression - the port always accrues the flat +1. Browser-verified live: five `hungerStep()` calls now raise `hunger` by exactly 5, where they previously raised it by 50. |
...
| `PotionOfFrost.shatter()` (`Freezing` blob: extinguish fire, `Chill`, eventual `Frost` immobilize) | `quaffPotion`'s `potionFrost` branch, `buffs.ts`'s `frost`, hero/monster turn gates | Simplified - the cast remains target-centred because no thrown-cell picker or persistent freezing-terrain layer exists, but it now applies the hero/target Chill effect, extinguishes Burning, and represents Java's explicit 10-turn Frost immobilization with a `frost` marker plus the shared paralysis lock when an already-capped Chill reaches its freeze transition. Freezing heaps, blob cadence, and full area geometry remain unported. **Corrected 2026-09-19:** the "elemental harmful-buff damage" half had no Java source (the potion only seeds Freezing, Freezing only chills) and even hit frost elementals with frost - removed. Type-check/build only; browser verification owed per ROADMAP.md section 10. |
