...
| Summoned newborn elemental scaling (`15 * regionScale` HP and friends) | `newbornElemental` row | Not ported: the row is flat where Java scales with depth region. Recorded by `MONSTER_ANALYSIS_SLIME_CAUSTIC_DM100_ELEMENTAL.md`. |
...
| SentryRoom Sentry `damage()` no-op invulnerability | - | Not ported: the HP-1 body survives only because evasion 1000000 defeats every attack roll; corner-case AoE/blob damage would wrongly kill it. Recorded by `MONSTER_ANALYSIS_DEMONSPAWNER_SENTRY_ROTHEART_ROTLASHER.md`. |
