...
| The other seven abilities: Mage `ElementalBlast`/`WildMagic`, Rogue `ShadowClone`, Huntress `SpiritHawk`, Duelist `Challenge`/`ElementalStrike`/`Feint` | - | **Not ported**, deliberately, and not offered: each needs systems this port does not have yet (per-wand blast factors and a wand-randomization pass; an ally actor for the clone and the hawk; a duel tracker; the four blade imbuements; a feint buff and its after-image). Their data rows exist - names, charge costs, targeting modes and T4 talent lists - so the ported/non-ported split is one set in `armorAbilities.ts`, and the choice panel offers only what can actually run. |
