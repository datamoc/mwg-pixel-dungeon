...
is genuinely per-item here, `ringBonusLevel`) and drops the hand-list's ceiling. Two classes stay
excluded on top of Java's predicate, and these *are* model gaps, each one a real Java target whose
upgrade would otherwise be a silent no-op: a carried **missile stack** (no per-stack level here)
and a **wand** (this port's wands share one bag id and take their power from `weaponLevel` - see
...
equipped pair plus the carried ring, offers neither the wand nor the missile stack nor the
runestone, and buying the ring's upgrade moves its own level 0 -> 1 for its real 1000 favor).
...
**Still open, and model-gated**: the consumed-missile set retirement above, and the missile half of
the reforge generally - the picker now excludes missile stacks outright, the same stated exclusion
the upgrade service makes, because a carried stack has no level of its own here (see the
`MissileWeapon` row and the dedicated roadmap item).
Harden keeps the weapon/armor list, which is what its own Java predicate asks for, and picks up
the same missile caveat for the same reason.
...
| `MeleeWeapon.ability()` overrides (all 30), the charge meter, `COUNTER_ABILITY` | `src/items/weaponAbilities.ts` (per-class table), `useWeaponAbility`/`resolveHeroAbilityAttack`/`tickWeaponAbility`, the `weaponAbility` action, persisted charge/windows | **Ported (2026-09-17).** Every cost and magnitude cross-checked against the shipped `ability_desc` catalogue strings, not re-read from Java: sneak 6/8/10 turns, heavy blow 35/45/40/30 +daze 5 (free on surprise), cleave 20/23/27/30/33 +free recast within 5 turns of a kill, spin +33%/spin to 3, guard 6/8 turns of full negate, combo 35/40/45 per recent hit, spike 30/45 ranged +knockback, lunge 35/67 with the step, harvest 100/80% bleed, sword dance +60% speed/+25% accuracy, defensive stance 3x evasion, retribution below half HP +50, charged shot force-hit +5x5 area, runic slash +300% enchant power, lash area attack. The T-key action spends charges through `weaponAbilityCost` (the ranked 0.5/1/1.5/2, tracker consumed with the spend - this also closes the Feint row's stale "inert tracker" clause), and charge, windows, spins, stance and shot state all persist with the run. **Three real bugs found and fixed auditing the table against the descs**: lunge staged knockback that neither lunge desc mentions (only spike's do); combo strike kept a +35% floor with no recent hit (the desc awards per hit, so zero hits is a plain guaranteed hit); thrown hits never reached the combo window (the desc counts "melee or thrown weapons" - the throw path shares the same choke point on a hero copy, so the accrual now keys on `isHero`). **Stated simplifications**: the charge economy (+1 per landed melee hit, +1 per 10 turns, cap 10) stands in for Java's exact accrual - which also means a landed 2-cost strike nets -1, pinned live rather than left implicit; damage strikes auto-target the nearest visible enemy where Java opens a cell selector; setup abilities cost one turn except sword dance/stance, which the descs free; spin lasts a fixed 3 turns. **No missile weapon has an ability at all** (zero `ability_desc` keys under `items.weapon.missiles.*`), correcting the roadmap's "Melee/Missile" phrasing - the crossbow's charged shot is melee-side (Java's `Crossbow extends MeleeWeapon`). Verified headlessly (30-class roster, lookup/case/fallback, discount/clamp/floor, spin math) and live (`tools/scratch/weapon-ability-livecheck.mjs`, 13/13: sneak spend + invisibility, spin costs/cap/warn, cleave kill refund + free recast, heavy net -1 + daze, rank-4 counter free + consumed, melee and thrown hits both feeding the combo window). |
...
boss-challenge-badge flag (Rankings work), and the flame burst (no one-shot hook in the pooled particle layers).
...
split - see the `MOB_LOOT` row's "found, not fixed" note); the dust wraith-curse still needs a Wraith mob.
...
and the shelf's per-row detail - STR requirements and wand charges stay unshown (no STRReq system, no per-heap
wand state). The shelf list itself still shows the whole shelf in one window (Java opens one `WndTradeItem` *per
