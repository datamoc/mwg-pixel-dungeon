...
| `GameScene`'s in-game menu (`WndGame`: continue / save / journal / badges / rankings / settings / exit to title), `GameScene.showBanner`/`Banner` (level-up, quest and boss banners), and `ui/Toast.java` | `main.ts`'s save/load key bindings (`KeyO`/`KeyP`), `src/ui/badgeBanner.ts`, `gameLog` + the action-bar `hintLabel` | **Not ported - checked against tag `v3.3.8` (2026-09-12), and the three parts are not one job.** There is **no in-game menu at all**: save and load sit on the `KeyO`/`KeyP` bindings instead of `WndGame`. That is the real gap here, it is bounded, and it is the natural next in-game user of `Window`/`WindowStack`, which the title screen and the journal already run on. `Banner` is missing as a general widget - only the *badge* banner exists (`ui/badgeBanner.ts`, wired at `awardBadge`) and level-ups log instead, so that file is the precedent to generalise. `ui/Toast.java` is **not applicable as designed**: in v3.3.8 it is used *only* by `GameScene.selectCell()` to show the active cell selector's own `prompt()` - one bottom-centred toast whose close button cancels the selection - and this port's targeting is creature-based with no cell-selector prompt at all, so there is nothing for it to display; the message roles are `gameLog` and the toolbar's `hintLabel`. (Java-style transitions are the `InterlevelScene` row above.) |
