...
| `FungalSentry` entire (poison extender, gas/poison immunities, ballistics reach, 200 HP) | - | Not ported at all. Recorded by `MONSTER_ANALYSIS_DEMONSPAWNER_SENTRY_ROTHEART_ROTLASHER.md`. |
...
| Honeypot shatter into Bee (pot-holder targeting, honeyed charm, strike-back aggro) | - | Not ported: the honeypot item exists but nothing shatters it, so the bee row is unreachable dead data. Recorded by `MONSTER_ANALYSIS_FETIDRAT_BEE_LARVA.md`. |
