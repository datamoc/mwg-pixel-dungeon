...
| `WandOfWarding.onZap()` / nested Java `WandOfWarding.Ward.zap()` actor | `useWardingWand`, `takeWardTurn`, persisted ward fields | Simplified but playable: casts create scheduled tier-1 wards within the Java energy budget (`2 + wand level`), wards fire always-hit `NormalIntRange(2 + wand level, 8 + 4*wand level)` attacks at visible hostiles, and the real tier 1–3 zap lifetimes and tier 4–6 self-damage are reproduced. Java's dedicated Ward NPC actor exists; this port represents it by a blue-tinted immovable skeleton carrier. Aimed-cell placement, existing-ward upgrade/heal interaction, exact line-of-sight collision, and ward dismissal UI use nearest-visible targeting because this port has no dedicated Ward type or cell picker. |
