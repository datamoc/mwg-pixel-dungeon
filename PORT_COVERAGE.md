...
| `WandOfWarding.onZap()` / nested Java `WandOfWarding.Ward.zap()` actor | `useWardingWand`, `takeWardTurn`, `monsters.ts`'s `ward` kind, persisted ward fields | Simplified but playable: casts create scheduled tier-1 `ward` actors within the Java energy budget (`2 + wand level`), wards fire always-hit `NormalIntRange(2 + wand level, 8 + 4*wand level)` attacks at visible hostiles, and the real tier 1–3 zap lifetimes and tier 4–6 self-damage are reproduced. The TypeScript actor has its own kind and state; only its sprite reuses the skeleton sheet because this checkout has no Ward art asset. Aimed-cell placement, existing-ward upgrade/heal interaction, exact line-of-sight collision, and ward dismissal UI use nearest-visible targeting because this port has no cell picker. |
