...
| `Char.attack()`'s tail ordering: `attackProc()` -> `enemy.damage()` (the `damage()` overrides, shields, HP) -> the execute mechanics | `main.ts`'s `attack()` | **Partly ported, with three ordering deviations found 2026-09-12 while placing the `damage()` curves, and deliberately not corrected in the same pass.** Java (`Char.java`, v3.3.8 lines 490-562) runs the weapon proc (`attackProc`, 505) *before* `enemy.damage(effectiveDamage)` (519); the defender's `damage()` overrides - `Pylon`/`Eye`/`Slime`/`DemonSpawner` and `SoiledFist`'s grass reduction alike - run *inside* that call; and the two execute mechanics run *after* it (`Preparation`/`prep.canKO`, 524, and `CombinedLethality`, 541), each setting `enemy.HP = 0` directly rather than routing through a damage value. This port's order is curves -> execute -> corrupting -> shields -> soiled-fist, so: (a) `Corrupting.proc` compares `damage >= defender.hp` *after* the curves, where Java compares the pre-`damage()` value - a Slime Java would corrupt survives here, because the curve has already cut the number; (b) `SoiledFist`'s grass reduction and the King's `DKBarrier`/DM-300's barrier pool both run *after* the execute, so a soiled Yog-fist or a shielded boss can survive a hit this port has already announced to the player as "executed", while Java's `HP = 0` is immune to every one of them; (c) the port merges Java's two separate mechanics into one `max()` threshold (`combined_lethality`'s `0.4*points/3` missing-HP fraction with the Assassin's `enhanced_lethality`), and omits `CombinedLethality`'s own gates - its `BOSS`/`MINIBOSS` exclusion and its "the attacking weapon changed since the tracker was armed" condition. Each is a behavioural change on a path with no unit coverage, so they are recorded with their Java line numbers rather than changed blind; `ROADMAP.md` section 11's `attack()` item carries the same note. |
