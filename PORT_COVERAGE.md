...
| The other eleven abilities: Mage `ElementalBlast`/`WarpBeacon`/`WildMagic`, Rogue `SmokeBomb`/`ShadowClone`, Huntress `SpectralBlades`/`NaturesPower`/`SpiritHawk`, Duelist `Challenge`/`ElementalStrike`/`Feint` | - | **Not ported**, deliberately, and not offered: each needs systems this port does not have yet (per-wand blast factors and a wand-randomization pass; a beacon actor with its own window; the real `Blindness` buff and the `NinjaLog` ally; an ally actor for the clone and the hawk; the spirit-blade projectile with a per-target damage multiplier; a growing-power tracker wired into the SpiritBow; a duel tracker; the four blade imbuements; a feint buff). Their data rows exist - names, charge costs, targeting modes and T4 talent lists - so the ported/non-ported split is one set in `armorAbilities.ts`, and the choice panel offers only what can actually run. |
